...
poly_fit <- construct_training_data(fitting = "SA2s", SA1s = SA1s, SA2s = SA2s)
...
ggsave( 
  ...
  filename = "Sydney_training_data_SA2_only.png", device = "png", width = 9, height = 4.1,
  path = "./img/"
)
...
ggsave( 
  ggarrange(plots$p_prob + labs(fill = "Prediction of the\nprobability process"), 
            plots$interval90_prob + labs(fill = "90% prediction-interval width\nfor the probability process"), 
            nrow = 1, legend = "top"),
  filename = "Sydney_SA1_predictions_SA2_only.png", device = "png", width = 9.5, height = 4.4,
  path = "./img/"
)
...
ggsave( 
  ggarrange(plots$p_mu + labs(fill = "Prediction of the\nmean process"), 
            plots$interval90_mu + 
              scale_fill_distiller(palette="BrBG", breaks = c(100, 200, 300)) + 
              labs(fill = "90% prediction-interval width\nfor the mean process"), 
            nrow = 1, legend = "top"),
  filename = "Sydney_SA3_predictions_SA2_only.png", device = "png", width = 9.5, height = 4.4,
  path = "./img/"
)
