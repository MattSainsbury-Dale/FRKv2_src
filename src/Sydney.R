...
pred_val <- pred$newdata[val_id, ]

## Coverage
coverage = mean((pred_val$Z_percentile_5 <= SA1_NSW_sub[val_id, ]$total_poverty_count) & (SA1_NSW_sub[val_id, ]$total_poverty_count <= pred_val$Z_percentile_95 ))
# 0.901308 ## using SA1s as training data  
# 0.908310 ## using mixture of SA1s and SA2s as training data

write.csv(data.frame(coverage = coverage), 
          "./results/Sydney_SA1_coverage.csv", 
          row.names = FALSE)

# ---- Plotting BAU (SA1) predictions ----
...
# ---- Prediction SA3 predictions ----


RNGversion("3.6.0")
set.seed(1)
pred <- predict(S, type = "mean", newdata = SA3_NSW_sub)
