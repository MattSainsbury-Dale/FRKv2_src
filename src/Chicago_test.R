...
## In preprocessing, we left out the data from the years 2010 and 2019. 
## To perform model validation on our BAU level predictions in 2010
...
binned_data <- FRK:::map_data_to_BAUs(chicago_crimes, 
                                      # sp_pols = ST_pred, 
                                      sp_pols = ST_BAUs, 
...
for (nres in 1) {
  for (link in c("square-root", "log", "identity")) {
    for (fs_by_spatial_BAU in c(TRUE, FALSE)) {
      
      # ---- Model fitting ----
      
      browser()
      
      basis <- auto_basis(STplane(), chicago_crimes_fit, tunit = "years", nres = nres)
      
      fn <- if(link == "square-root") sqrt else if (link == "log") log else if (link == "identity") I 
