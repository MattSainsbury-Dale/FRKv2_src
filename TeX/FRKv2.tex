...
We demonstrate innovative features in this version of \pkg{FRK}, highlight its ease of use, and compare it to alternative packages, using both simulated and real data sets. 
}

%% - \Keywords{} with LaTeX markup, at least one required
%% - \Plainkeywords{} without LaTeX markup (if necessary)
%% - Should be comma-separated and in sentence case.
\Keywords{areal data, basis functions, big data, change-of-support, non-Gaussian, spatial statistics, spatio-temporal statistics}
\Plainkeywords{areal data, basis functions, big data, change-of-support, non-Gaussian, spatial statistics, spatio-temporal statistics}
...
% In their seminal work, \cite{Diggle_1998_spatial_GLMM} employed a stationary model for $Y(\cdot)$ within the spatial GLMM framework and a Markov chain Monte Carlo (MCMC) algorithm to obtain predictive distributions. 
 Prediction or estimation of unknown quantities from $n$ observations entails the inversion of an $n\times n$ covariance matrix for classical models. 
 Since this task is generally $O(n^3)$ in computational complexity, some form of dimension-reduction is required in `big data' settings. 
...
\citet{Bradley_2018_computationally_efficient_multivariate_ST_models_for_high-dimensional_count-valued_data}, \citet{Bradley_2019_ST_models_for_big_multinomial_data_using_conditional_multivariate_logit_beta_distribution}, and \citet{Zhang_2020_spatio-temporal_Arctic_sea_ice}. 
 ...
 The main purpose of this article is to present a major advance to \pkg{FRK} that allows it to cater for many distributions within the exponential family using the spatial GLMM framework; we henceforth refer to it as \pkg{FRK} v2. 
...
The process layer, which governs the conditional mean of the data, retains many similarities to that in previous releases of the package (henceforth referred to as \pkg{FRK} v1), as described by \cite{FRK_paper}. 
 Note that here we consider the spatial case only; the extension to a spatio-temporal setting is outlined in Section \ref{sec:spatio-temporal}. 
...
Accounting for only large and medium-to-small-scale spatial variation can result in overly optimistic predictions; this problem is alleviated by including a fine-scale-variation random process, $\xi(\cdot)$, that has very small-scale spatial correlation.
...
BAUs are a key element of \pkg{FRK} v1/v2, as they provide a framework that allows one to easily consider point-referenced and areal data simultaneously, and they facilitate solutions to spatial change-of-support problems. 
 Define the latent spatial process $Y(\cdot)$ evaluated over the $N$ BAUs as $Y_i \equiv Y(A_i)$, $i = 1, \dots, N$. After discretisation via the BAUs, we obtain the vectorised version of (\ref{eqn:04-01:Y(s)}):  
\begin{equation}\label{Ch4:eqn:vecY}
    \vec{Y} = \vec{T}\vec{\alpha} + \vec{S}\vec{\eta} + \vec{\xi},
\end{equation}
 where $\vec{Y} \equiv (Y_i : i = 1, \dots, N)^\tp$ is an $N$-dimensional vector of the process corresponding to the BAUs, $\vec{T}$ and $\vec{S}$ are known design matrices, and $\vec{\xi}$ is the vector associated with the fine-scale process.
...
As in \pkg{FRK} v1, the elements of $\vec{\xi}$ are modelled as independent and identically distributed (i.i.d.) Gaussian random variables with variance $\sigma^2_\xi$, and $\vec{\eta}$ is modelled as a mean-zero multivariate-Gaussian random variable with covariance matrix $\cov{\vec{\eta}}{\vec{\eta}}$. 
 ...
 Both formulations use block-diagonal matrices, so that basis-function coefficients between basis-function resolutions are independent; see Appendix \ref{Appendix:CovarianceTapering} for how we model the intra-resolution dependencies with $\vec{K}$ and $\vec{Q}$. 
...
\]
Therefore the mean process evaluated over the BAUs is $\mu_i = g^{-1}(Y_i)$,  $i = 1, \dots, N$, where $g^{-1}(\cdot)$ is the inverse link function, and we define $\vec{\mu} \equiv (\mu_i: i = 1, \dots, N)^\tp$.

Two distributions considered in this framework, namely, the binomial and the negative-
binomial distributions, have a known constant `size' parameter and a `probability of success'
parameter associated with every datum. 
 For these distributions, we also define a probability process, $\pi(\cdot)$, which is a function of the process, $\mu(\cdot)$, and the known size parameter: See Appendix \ref{Appendix:Distributions with size parameters} for further details.
...
\subsection{The data layer}\label{subsection:04-02:DataLayer}
 ...
 In practice, the spatial supports may not coincide with entire BAUs; when this is the case, we assume that a spatial support contains a BAU if and only if there is non-empty intersection between the BAU and the spatial support.
 ...
 Figure \ref{fig:BAU_intuition} shows a simple example illustrating how $D$ is discretised into BAUs, and how $D^O$ is derived from the spatial supports of the data.
...
\end{scope}
\end{tikzpicture}
\caption{An illustration of how the continuous spatial domain, $D$, is discretised into the set of  BAUs, $D^G$, and how the observation domain, $D^O$, is derived from the observation supports.
(Left panel) The continuous spatial domain, $D$.
(Centre-left panel) The spatial domain discretised into $N = 12$ BAUs, $D^G$. 
(Centre-right panel) The discretised domain superimposed with $m = 3$ observations, two of which are area-referenced, and one that is point-referenced. 
...
In \pkg{FRK} v2, the weights $w_{ij}$ may be controlled through the \code{wts} field of the BAUs object and the argument \mbox{\code{normalise\_wts}}. 
 ...
 By default (and implicit in \pkg{FRK} v1), each $v_i$ is set to 1. 
  The argument \mbox{\code{normalise\_wts}} controls whether $\vec{C}_Z$ corresponds to a weighted sum or a weighted average. If set to \code{FALSE}, then $w_{ij} = v_i$ for all $j$; if set to \mbox{\code{TRUE}} (implicit in \pkg{FRK} v1), then the $w_{ij}$ are normalised so that each row of $\vec{C}_Z$ sums to 1.  
...
\end{gather}
 ...
 Note that \pkg{FRK} v2 is backwards compatible, since an identity link function and a Gaussian data model yields the model used in \pkg{FRK} v1. 
...
This makes prediction of $\vec{u}$ and any function of $\vec{u}$ straightforward via MC simulation; see Section \ref{subsection:04-03:Prediction}. 
...
Note that all unknown quantities are treated as random in \pkg{TMB} (with a flat prior assumed if a prior is not provided).
 To retain the mixed-model interpretation of \pkg{FRK} v1, we fix the parameters and fixed effects to their posterior-mode estimates, and subsequently treat them as non-random quantities.
...
There are three quantities that could be of interest to the user: The latent process $Y(\cdot)$, the  mean process $\mu(\cdot)$, and data at unobserved locations. 
...
Recall that the Laplace approximation implies that, approximately, $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$; since $\vec{Y}$ is a linear function of $\vec{u}$, approximate inference on $Y(\cdot)$ can hence be done using well-known formulas. 
  However, the posterior distribution of a non-linear function of $Y(\cdot)$ (e.g., the mean process) is typically not available in closed form, and some approximation is required.
  In \pkg{FRK} v2, we use a Monte Carlo (MC) framework.
...
where every column of the matrix $\vec{A}$ is the estimated posterior mode of $\vec{\alpha}$, and each of the $n_{\text{MC}}$ columns of the matrix $\vec{U}$ are draws from  $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$.  
 We obtain MC samples of $\vec{\mu} \mid \vec{Z}, \vec{\theta}$ via $\vec{M} \equiv g^{-1}(\vec{Y}_{\!\text{MC}})$, where $g^{-1}(\cdot)$ is applied element-wise. Finally, MC samples of the noisy data can be constructed straightforwardly using $\vec{M}$, since the distribution of an exponential family member depends only on its conditional mean (and possibly a dispersion parameter, which we model as being constant throughout the spatial domain). 
...
\subsubsection{Arbitrary prediction regions}
 ...
 These regions may overlap and may not coincide with entire BAUs: Our criteria for determining whether a region contains a BAU is the same as that used for the spatial supports originally associated with the observations (see Section \ref{subsection:04-02:DataLayer}). 
 We then define the set of prediction regions as $D^P \equiv \{\tilde{B}_k : k = 1, \dots, N_P\}$, where $\tilde{B}_k \equiv \cup_{i\in \tilde{c}_k} A_i$, and where $\tilde{c}_k$ is a non-empty subset of $\{1, \dots, N\}$ that gives the indices of the BAUs associated with the $k$th user-specified region.
...
Since aggregation must be done on the response scale, we restrict prediction over arbitrary regions to the mean process and the noisy data.  
...
As in Section \ref{subsection:04-02:DataLayer}, the proportionality constants $\{v_i: i = 1, \dots, N\}$ for the weights $\{\tilde{w}_{ik}\}$ are controlled by the \code{wts} field of the BAU object and the argument \mbox{\code{normalise\_wts}}. 
 For consistency between the model fitting and prediction stages, in \pkg{FRK} v2 we require that the same proportionality constants and the same setting of \code{normalise\_wts} are used in construction of both $\vec{C}_Z$ and $\vec{C}_P$. 


 MC samples of $\vec{\mu}_P \mid \vec{Z}, \vec{\theta}$ can be constructed  via $\vec{M}_P \equiv \vec{C}_P \vec{M}$, where recall that $\vec{M}$ consists of MC samples of $\vec{\mu} \mid \vec{Z}, \vec{\theta}$. Predictions and uncertainty quantification of the predictions can then be computed straightforwardly from $\vec{M}_P$. 
 When one wishes to predict data over aggregations of BAUs, \pkg{FRK} v2 simulates from the specified response distribution with mean equal to the samples in $\vec{M}_P$, and then returns predictions and uncertainty quantification of these simulated data. 
...
In these situations, a better fit to the data may be obtained by allowing each spatial BAU to be associated with its own fine-scale variance parameter. 
...
We now demonstrate the new features in \pkg{FRK} v2, an overview of which is presented in Table \ref{tab:new_arguments_in_functions}. 
 ...
 In Section \ref{sec:3:increased_resolution}, we show the potential improvement in predictive performance of \pkg{FRK} v2 over \pkg{FRK} v1 when the data are Gaussian, thanks to an increased number of basis functions in \pkg{FRK} v2. 
...
%        & \code{newdata} & The prediction regions; in addition to \code{Spatial*DataFrame}and \code{STFDF}, \code{newdata} can now be a \code{SpatialPoints} or \code{STI} object, facilitating prediction over spatial or spatio-temporal point-referenced locations.\\
        ...
        The inclusion of \code{"link"}, \code{"mean"}, and \code{"response"} in \code{type}, respectively, indicate that inference on $Y(\cdot)$, $\mu(\cdot)$, or the noisy data, is made.\\ 
...
%        & \code{k} & Vector of size parameters at each prediction location (applicable only for binomial and negative-binomial data).\\
        ...
        The spatial BAUs in a spatio-temporal setting, where spatio-temporal BAUs are constructed by taking the Kronecker product of the spatial BAUs and temporal box functions. If \code{NULL}, the spatial BAUs are constructed automatically from the data.\\
...
The true mean process over $D$ is also shown in Figure \ref{fig:Poisson_true_and_Z}.
...
The second element is a \class{list} of matrices containing MC samples for each term in \code{type} at each prediction location. Finally, we can generate a \class{list} of \class{ggplot} \citep{Wickham_2016_ggplot2} objects of the predictions and their associated uncertainty using the function \fct{plot}: 
\begin{Code}
R> plots <- plot(S, pred$newdata)
\end{Code}
%$
The \class{ggplot} objects can be arranged easily in a grid using various dedicated packages; we used \pkg{ggpubr} \citep{Kassambara_2020_ggpubr}. Figure \ref{fig:Poisson_nres3} shows predictions and prediction-interval widths for the latent process $Y(\cdot)$ and the mean process $\mu(\cdot)$. 
The predictions of $\mu(\cdot)$ are reasonable given the data and the true process shown in Figure \ref{fig:Poisson_true_and_Z}. 
The prediction-interval width for $Y(\cdot)$ is relatively flat, and larger in regions of data paucity and along the boundary of the spatial domain; on the other hand, the prediction-interval width for $\mu(\cdot)$ is large when $\mu(\cdot)$ is large, as can be expected when the response is Poisson distributed. 
...
In this section, we illustrate \pkg{FRK} v2 on simulated negative-binomial, areally-referenced, spatial data, as well as its use in predicting over large areas (i.e., large spatial supports). 
...
\begin{figure}[t!]
    ...
    \vspace{-20pt}
    \caption{Prediction and prediction-interval width for the simulated negative-binomial areal data set. (Left panel) Prediction of the probability process, $\pi(\cdot)$. (Centre-left panel) Width of the 90\% central prediction interval for $\pi(\cdot)$. (Centre-right panel) Prediction of the mean process, $\mu(\cdot)$. (Right panel) Width of the 90\% central prediction interval for $\mu(\cdot)$.
}   
...
In contrast, the prediction-interval width for $\pi(\cdot)$ is low when the prediction is near 0 or 1, and it increases when the prediction is near 0.5: This is expected from properties of the negative-binomial distribution. 
 ...
 The mean empirical coverage from the 90\% prediction intervals was 90.9\%, which is almost nominal and very reasonable given the difficulty inherent to spatial change-of-support problems.
...
\end{Code}
Recall from Section \ref{subsection:04-03:Prediction} that we do not allow predictions of $\pi(\cdot)$ over arbitrary polygons.
...
\begin{figure}[t!]
	...
	\vspace{-20pt}    
    \caption{Prediction (left panel) and prediction-interval widths (right) for the mean process, $\mu(\cdot)$, when predicting over arbitrary polygons in the example of Section \ref{sec:03-03:negative-binomial}. Note that these polygons have equal area.}   
...
\begin{table}[t!]
    \centering
    \caption{Diagnostics comparing the predictive performance when using a range of basis-function resolutions with point-referenced count data. The diagnostics are the root mean-square prediction error (RMPSE), the continuous ranked probability score (CRPS), and the empirical coverage (Cvg90) and interval score (IS90) resulting from a central prediction interval with a nominal coverage of 90\% (see Appendix \ref{app:ScoringRules} for further details). The diagnostics are with regard to prediction of the true mean process, $\mu(\cdot)$, and they are averaged over all unobserved locations.
...
The data used in that study was made up of a training and a test set consisting of 105,569 observations and 42,740 observations, respectively. 
...
The results show that the increased number of basis functions significantly improves the diagnostic scores of \pkg{FRK} v1, and the results for \pkg{FRK} v2 are now comparable to those of MRA.  
...
\pkg{FRK} v2 also caters for non-Gaussian, point- and areally-referenced, \textit{spatio-temporal} data. 
For the sake of brevity, we will not use a simple example to show this, and instead refer readers to the application given in Section \ref{sec:ST_example}. 
...
For this comparative study, data pre-processing involved first coarsening the image from over 10 million pixels to a more manageable 33,750 pixels, by creating a 150 $\times$ 225 grid and computing the mean value of the response within each grid cell. 
Then, as the data provided by the MODIS instrument is continuous (measuring spectral radiances in units of $\text{W/m}^2/\si{\um}/\text{st}$), we applied a reasonable threshold to obtain a binary version of the data (i.e., Cloud, or No-cloud).
...
\begin{figure}[t!]
    ...
    \vspace{-25pt}
     \caption{
     MODIS data used in the comparative study of Section \ref{sec:04-01:MODIS}; a blue background is used to denote missing data. (Left panel) The missing-at-random (MR) sample used for training.  (Centre-left panel) The MR test data. (Centre-right panel) The `missing-in-a-block' (MB) sample used for training. (Right panel) The MB test data. 
...
The software used in this study each required several modelling decisions, which had to be made in a way that balanced predictive performance and run time. 
 We took a systematic approach to model selection by splitting the training set in two, and then using one half for model fitting and the other half for model validation. 
 ...
 \pkg{mgcv}, we used the \fct{bam} function, which is similar to the generalised additive model function \fct{gam} but optimised for large data sets, with 3,000 knots. 
For \pkg{spBayes}, we used 400 knots; increasing the number of knots further was computationally prohibitive. 
 ...
 The packages \pkg{spNNGP} and \pkg{spBayes} use Markov chain Monte Carlo (MCMC): At both training and test locations, we used 10,000 total MCMC samples, a burn-in of 6,000, and a thinning factor of 10; hence, 400 approximately independent samples from the predictive distribution of the process were available at each spatial location. 
...
The predictions of \pkg{FRK}, \pkg{INLA}, and \pkg{spNNGP} are similar, 
...
However, the underlying distribution of the probability process is unidentifiable, as the predictive distribution, ${Z^*} \mid \vec{Z}$, for some validation datum ${Z^*}$, depends only on the posterior expectation of the probability parameter at the corresponding location, $\ENoLR{\pi^* \mid \vec{Z}}$.
...
For the MR scheme, there is little discernible difference between \pkg{FRK}, \pkg{INLA}, \pkg{mgcv}, and \pkg{spNNGP}. 
 ...
 The task of prediction over a completely unobserved region is challenging, and so it is no surprise that the diagnostics for the MB scheme are significantly worse than for the MR scheme. 
 ...
 Note that all run times increased under the MB scheme; however, \pkg{FRK} increased by the smallest amount (increasing by a factor of less than 2), while the run times for other packages increased by between-a-factor-of 3 and 10. 
 ...
 Overall, these results suggest that \pkg{FRK} is comparable to or better than other packages in this application. 
 The main advantages of \pkg{FRK} however lie in the ease with which it allows one to do other more elaborate analyses with spatial or spatio-temporal non-Gaussian data, as shown in the next sections. 
...
\begin{figure}
    ...
    \includegraphics[width = \linewidth]{img/Am_data_and_blocks.png}
        \vspace{-20pt}
    \caption{Americium soil data. The `$\times$' denotes Ground Zero (GZ), where the devices were detonated. (Left panel) Am concentrations on the original scale. (Centre panel) Am concentrations on the log scale. (Right panel) Americium soil data blocking schemes: Scheme 1 (red), centred away from GZ, and Scheme 2 (blue), centred on GZ. When the data locations overlap, we show only one measurement.}   
  \label{fig:Am_data}
\end{figure}

As in \cite{Paul_Cressie_2011_lognormal_kriging_block_prediction}, we use a piecewise linear trend, where observations within a distance of 30.48m from GZ are assumed to follow a different trend to those observations beyond 30.48m from GZ. 
...
% \red{The following code constructs the BAU-level covariates needed to fit this piecewise linear trend: \code{BAUs\$x1} and \code{BAUs\$x2} are used to model the intercept and slope for distances < 30.48m from GZ, respectively,  and \code{BAUs\$x3} and \code{BAUs\$x4} are used to model the intercept and slope for distances $\geq$ 30.48m from GZ, respectively.}
 The following code constructs the BAU-level covariates needed to fit this piecewise linear trend: \code{BAUs\$x1} and \code{BAUs\$x3} are indicator variables used to model the intercepts in each region, and \code{BAUs\$x2} and \code{BAUs\$x4} are used to model the slopes in each region. 
...
In order to generate block-level predictions, we pass a \class{SpatialPolygonsDataFrame} object into the \code{newdata} argument of \fct{predict}. 
 In the following code, \code{blocks} is a \class{SpatialPolygonsDataFrame} object containing the polygons corresponding to the 2 blocking schemes shown in Figure \ref{fig:Am_data}:
...
To validate our predictions, we used the \proglang{R} package \pkg{georob} \citep{georob}, which implements an approximately unbiased back-transformation of kriging predictions of log-transformed data \citep{Cressie_2006_block_kriging_lognormal_spatial_processes}. 
Kriging does not scale well for large sample sizes, however %as
 the size of this data set is sufficiently small for kriging to be possible. %, \pkg{georob} can be used to validate the predictions and associated uncertainty quantification obtained using \pkg{FRK} v2. 
...
In this example, we consider a region of New South Wales containing 7,909 SA1 regions, 180 SA2 regions, and 31 SA3 regions, and we aim to infer `poverty' levels at the SA1 and SA3 regions 
...
\includegraphics[width = \linewidth]{img/Sydney_training_data_mixed.png}
    \vspace{-20pt}    
    \caption{
    Training data used for modelling the number (or proportion) of families `in poverty' (see Appendix \ref{Appendix:Sydney_data_description} for how we define `in poverty'). 
...
The predictions and associated uncertainty over the SA1 regions are shown in Figure \ref{fig:SA1_predictions}, which was generated using \fct{plot}.
 \begin{figure}[t!]
    ...
    Solid-grey regions correspond to SA2 regions in which the total number of families is zero, and hence are omitted from the study. 
...
\end{figure}
 ...
 If only coarse-resolution data are available (i.e., all data supports are associated with multiple BAUs), in order to avoid identifiability issues, \pkg{FRK} v2 automatically fixes $\sigma^2_\xi$ prior to fitting the model with \pkg{TMB}. 
...
%The data considered in this analysis consisted of 1,748,360 crimes. 
 We considered only crimes labelled as assault or battery; roughly 1.75 million crimes in total. 
 Note that the default behaviour of all releases of \pkg{FRK} is to bin data falling into the same BAU; in this example, the final number of observations post-binning is 1444.  
...
Spatio-temporal BAUs may then be constructed by passing the CAs and data into \fct{auto\_BAUs}. Recall that, in this case, the spatio-temporal BAUs are space-time volumes constructed by taking the Kronecker product of the spatial BAUs and temporal box functions. 
\begin{Code}
...
\begin{figure}[t!]
    ...
    \vspace{-15pt}    
    \caption{Predictive distributions in the validation years (2010 and 2019) for three randomly-selected CAs (Archer Heights, Ashburn, and Roseland). 
...
For both years, Figure \ref{fig:chicago_validation_predictions} shows agreement between the predicted and actual number of crimes. Furthermore, the prediction uncertainty is roughly proportional to the predicted value, as one expects when modelling counts.
For these validation years, we also computed the empirical coverage when using 90\%, 80\%, 70\%, and 60\% prediction intervals, and the mean absolute percentage error (MAPE; see Appendix \ref{app:ScoringRules}). 
...
The slightly worse results in the year 2019 is expected, as forecasting into the future is, in general, a harder task than predicting within the time span of the data.
...
For illustration, we next focus on three randomly-selected CAs: Ashburn, Roseland, and Archer Heights. The time series of the observed data, predictions, and 90\% prediction intervals for these CAs, are shown in Figure \ref{fig:chicago_time_series}. 
...
In this paper, we have described  \pkg{FRK} v2, an extension to the \proglang{R} package \pkg{FRK}, that allows for the spatial and spatio-temporal modelling of, and large-scale prediction from, big, non-Gaussian data. 
...
The existing functionality of \pkg{FRK} is retained with this extension; in particular, the package makes use of automatic basis function construction, is capable of handling both point-referenced and areal data, and facilitates the so-called spatial change-of-support problem through the use of BAUs.
The package now provides a highly accessible and user-friendly approach to spatial and spatio-temporal modelling of big data in both a Gaussian and non-Gaussian setting.
 ...
 Another requirement is the practical necessity to fix the fine-scale variance parameter in spatial change-of-support applications; note that this is not an issue if one is able to obtain a reliable estimate through other means (e.g., via previous census data or via quality-control experiments). 
 We are currently exploring avenues to relax this requirement via the provision of a robust offline estimate.
 A limitation of \pkg{FRK} v2 is that, despite the added flexibility, several models of interest, such as the zero-inflated Poisson, are still not catered for. 
...
Recall from Section \ref{subsection:04-01:ProcessLayer} that \pkg{FRK} v2 allows the basis-function coefficients, $\vec{\eta}$, to be parameterised using either a prior covariance matrix, $\vec{K}$, or using a prior precision matrix, $\vec{Q}$.  
 In this appendix, we describe the parameterisation of these matrices.
 Both $\vec{K}$ and $\vec{Q}$ are block-diagonal matrices, wherein basis-function coefficients within a basis-function resolution are dependent, but independent across different resolutions. Hence, $\vec{K}$ and $\vec{Q}$ are fully defined via their intra-resolution dependencies. 
...
We note that $\vec{Q}_k$ is diagonally dominant, and hence it is positive-definite. 
 ...
 Note that \pkg{LatticeKrig} uses $\vec{Q}_k^\tp\vec{Q}_k$ as the precision matrix blocks, whereas we use $\vec{Q}_k$.  
...
Two data models that can be used with \pkg{FRK} v2, namely, the binomial and the negative-binomial distributions, have a known constant `size' parameter $k_j$ and a `probability of success' parameter, $\pi_j$, associated with every datum $Z_j$.
