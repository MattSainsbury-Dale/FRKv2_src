...
% Since this task is generally $O(m^3)$ in computational complexity, some form of dimension-reduction is often employed in `big data' settings. 
 \Copy{FixedRankMethods}{Since this task is generally $O(m^3)$ in computational complexity, alternative approaches that scale well to large sample sizes, which we collectively refer to as \textit{fixed-rank} approaches, are often employed in `big data' settings.}
...
\citet{Bradley_2018_computationally_efficient_multivariate_ST_models_for_high-dimensional_count-valued_data}, \citet{Bradley_2019_ST_models_for_big_multinomial_data_using_conditional_multivariate_logit_beta_distribution}, and \citet{Zhang_2020_spatio-temporal_Arctic_sea_ice}. 
 ...
 \Copy{KrigingClause}{Despite the many modelling approaches available, software for spatial and spatio-temporal modelling of non-Gaussian data (for which kriging-based approaches may be sub-optimal) are relatively limited.} 
 \Copy{SoftwareReview}{
 Software 
...
%Accounting for the scales of spatial variation with only $\vec{t}(\cdot)^\tp \vec{\alpha}$ and $v(\cdot)$ can result in an overly smooth spatial model and hence overly optimistic predictions; this problem is alleviated by also including a fine-scale-variation random process, $\xi(\cdot)$. 
 \Copy{MotivationForFineScaleProcess}{If the scales of spatial variation in the modelled spatial process are at a fine resolution, including only $\vec{t}(\cdot)^\tp \vec{\alpha}$ and $v(\cdot)$ can result in an overly smooth spatial model and hence overly optimistic predictions; this problem is alleviated by also including a fine-scale-variation random process, $\xi(\cdot)$.}
...
% These BAUs may correspond to physical real-world boundaries, such as regional borders, or simply a fine mesh covering that spatial domain. 
 \Copy{BAUEffectOnPrediction}{If $N$ is large enough to render the discretisation error neglible, the use of BAUs does not result in a loss in predictive power compared with classical geostatistical approaches (see \citeauthor{FRK_paper}, \citeyear{FRK_paper}, Sec.~4;  \citeauthor{Cressie_2021_review_spatial-basis-function_models}, \citeyear{Cressie_2021_review_spatial-basis-function_models}, Sec.~5), and this allows \pkg{FRK} to be used as an omnibus approach for spatial analyses.}
...
% This allows $\vec{u}$ to be integrated out of $L(\vec{\theta}; \vec{Z}, \vec{u})$, yielding the Laplace approximation of the observed-data log likelihood, $\ell^*(\vec{\theta}; \vec{Z}) \approx \ell(\vec{\theta} ;  \vec{Z}, \hat{\vec{u}}) + \frac{{p}}{2} \ln{2\pi} + \frac{1}{2} \ln{\left|\vec{H}\right|}$.
   \Copy{LaplaceApproximation}{For a more detailed discussion on the Laplace approximation and its accuracy, see, for example, \cite{Tierney_1986_Laplace_approx}, \cite{Rue_Martino_2007}, and \cite{Rue_2009_INLA}.}
...
%  To this end, an effective strategy is to plot the residuals in space. Figure~\ref{fig:Poisson_residuals} shows the residuals in space for two models, which were fit using one and three resolutions of basis functions, respectively. The residuals for the first model show strong spatial dependence, indicating that the model has not adequately captured the spatial dependence in the data; in contrast, the residuals for the second model show almost no spatial dependence, indicating that a satisfactory number of basis functions has been used. 
 ...
 Table~\ref{tab:03-02:PoissonScoringRules} shows the results for each model. 
  ...
  Further, \Copy{PoissonVaryingBasisFunctions}{the relative smoothness in the data (recall the right panel of Figure~\ref{fig:Poisson_residuals}) suggests that only a moderate number of basis functions are needed for this application. 
...
For this comparative study, we pre-processed the data as follows. First, we coarsened the image from over 10 million pixels to a more manageable 33,750 pixels, by creating a 150 $\times$ 225 grid and computing the average value of the response within each grid cell. 
 \Copy{MODISthreshold}{Second, since the data provided by the MODIS instrument is continuous (namely spectral radiances in units of $\text{W/m}^2/\si{\um}/\text{st}$), we applied a threshold to obtain a binary version of the data. Specifically, pixels with spectral radiance greater than 7,000 $\text{W/m}^2/\si{\um}/\text{st}$ were labelled `Cloud', and the remaining pixels were labelled `No-cloud'. This threshold was chosen for illustration purposes and has no particular scientific meaning.}  
...
\Copy{spBayesLimitation2}{For \pkg{spBayes}, we used 400 knots; increasing the number of knots further was computationally prohibitive (recall from Section~\ref{sec:intro} that \pkg{spBayes} uses basis functions that depend on covariance-function parameters, so that computationally it can only handle a small number of predictive-process knots).} 
...
The predictions from \pkg{FRK}~v2, \pkg{INLA}, and \pkg{spNNGP} are similar, 
 ...
 \Copy{SmallBand}{Close inspection of the corresponding training data, shown in the centre right panel of Figure~\ref{fig:03-04-Modis1}, indicates that predictions within the missing block are largely driven by observations immediately surrounding the block.} 
...
% it is a negatively oriented rule, where 
 ...
 \Copy{DataPaucityExplanation}{These results illustrate that purely covariance or nearest-neighbour based spatial models are only suitable when prediction locations are `close' to data points. Spatial models that include covariates are typically much more effective at predicting over regions of data paucity.}
...
\Copy{lognormalkriging}{In order to mimic lognormal block kriging, which models the response as a lognormal process, here we fix the measurement-error standard deviation to a small value relative to the process variance (specifically, we set the measurement-error standard deviation equal to 1) prior to model fitting.} 
...
Using a GLMM model and the software \pkg{TMB}, \pkg{FRK}~v2 can now cater for many distributions within the exponential family, as well as many link functions. % (Table~\ref{table:response_and_links}).  
 \Copy{MoreBasisFunctions}{Furthermore, \pkg{FRK}~v2 allows for the use of many more basis functions when modelling the spatial process, and hence it can often achieve more accurate predictions in a Gaussian setting than \pkg{FRK}~v1 (Table~\ref{tab:Heaton_comparison:full}).} 
 ...
 \Copy{FutureWork}{Further future work includes the addition of multivariate responses, for which basis-function models are well suited \citep[for an overview, see][Sec.~4.1]{Cressie_2021_review_spatial-basis-function_models}, as well as spatially varying regression coefficients, which can enhance understanding of the underlying spatial process.}
 %TODO Yang (2022) noted several problems with simulation-based residuals. When the model has at least one continuous covariate, Yang proposed an interesting new form of residual. 
