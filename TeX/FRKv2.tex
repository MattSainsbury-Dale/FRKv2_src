...
%TODO Random symbols at the beginning of scripts/MODIS_modelling_fns/INLA.R
%TODO  .tar.gz file that weren't updated but I'm happy for you to submit and these could be changed later (please make note) maybe even after acceptance... namely the figures need to be made the same as in the paper (currently they're different) and files could be cleaned up (e.g., #OUTPUT#).
%TODO We also need to remove those funny symbols in the beginning of INLA.R in the conda version; that should be done relatively soon.
...
In \FRKgeneric, the medium-to-small-scale term $v(\cdot)$ is constructed as a linear combination of $r$ spatial basis functions with random coefficients, where $r$ is fixed and usually smaller than $m$, the number of observations. Specifically,
\[
v(\vec{s}) 
...
% These BAUs may correspond to physical real-world boundaries, such as regional borders, or simply a fine mesh covering that spatial domain. 
 \Copy{BAUEffectOnPrediction}{The finest resolution at which one can make predictions is at the level of the BAU. Point predictions are not possible; however, when the BAUs are so fine that the chosen basis functions are approximately constant within each BAU, the BAU-level predictions are practically equivalent to point predictions (see \citeauthor{FRK_paper}, \citeyear{FRK_paper}, Sec.~2, for more details). %, and see \citeauthor{FRK_paper}, \citeyear{FRK_paper}, Sec.~4, and   \citeauthor{Cressie_2021_review_spatial-basis-function_models}, \citeyear{Cressie_2021_review_spatial-basis-function_models}, Sec.~5).
 }
 Now, let $Y(A_i)$ denote a representative value of $\{Y(\vec{s}) : \vec{s} \in A_i\}$, where commonly that value is the spatial integral or the spatial average over $A_i$.  
