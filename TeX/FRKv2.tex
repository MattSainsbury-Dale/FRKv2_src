...
%\green{Finally, we note that two distributions considered in this framework, namely, the binomial distribution and the negative-binomial distribution, have a known constant `size' parameter and a `probability of success'
%parameter associated with every datum. 
% For these distributions, we also define a probability process, $\pi(\cdot)$, and its discretised version evaluated over the BAUs, $\vec{\pi}$; see Appendix \ref{Appendix:Distributions with size parameters} for further details. }

\green{A caveat to the above description of the mean process is that two distributions considered in this framework, namely, the binomial distribution and the negative-binomial distribution, have a known constant `size' parameter and a `probability of success'
parameter associated with every datum. 
...
% Figure \ref{fig:BAU_intuition} shows a simple example illustrating $D$ is discretised into $N = 12$ BAUs, and how $B_1$, $B_2$, and $B_3$ are defined from $m=3$ observations, resulting in $D^O \equiv \{B_1, B_2, B_3\}$.
 ...
 \green{
% We define the \textit{data process}, $Z(\cdot) \sim \text{EF}(\mu(\cdot), \psi)$, which follows some distribution in the exponential family (discussed below) that depends on the mean process over the corresponding spatial support, and possibly a dispersion parameter (discussed below). 
 % Then, the observed data, $Z_j$, $j = 1, \dots, m$, is a specific realisation from $Z(R_j)$ which, in practice, is assumed to be a realisation from $Z(B_j)$. 
 We denote the vector of $m$ observations (the data vector) as $\vec{Z} \equiv \left(Z_1, \dots, Z_m\right)^\tp$. 
% where $Z_j \equiv Z(R_j)$ is a specific realisation from the data process over the spatial support $R_j$, $j = 1, \dots, m$. 
  Each datum is originally associated with a spatial support, $R_j$, $j = 1, \dots, m$, which we associate to one or more BAUs. 
% We associate the original spatial supports, $R_j$, $j = 1, \dots, m$, with one or more BAUs.
...
%  Attributing point-referenced data to BAUs is straightforward.
  We then define the set of observation supports as $D^O \equiv \{B_j : j = 1, \dots, m\} $, where $B_j \equiv \cup_{i\in c_j} A_i$ is a discretised representation of $R_j$. 
...
%  that is, when $Z_j \sim \text{Bin}(\mu_{Zj}; k_{Zj})$ or $Z_j \sim \text{NB}(\mu_{Zj}; k_{Zj})$, $j = 1, \dots, m$, 
  we enforce $v_i = 1$, $i = 1, \dots, N$, and we  enforce \mbox{\code{normalise\_wts = FALSE}}. This means that aggregation over the BAUs is a simple, un-weighted summation, which allows us to construct the vector of size parameters over the observation supports, $\vec{k}_Z$, by summing over the vector of size parameters at the BAU level, $\vec{k}$: That is,  $\vec{k_Z} = \vec{C}_Z \vec{k}$.}
...
% Note that the negative-binomial distribution has many parameterisations: In FRK v2, we assume that $Z_j$ represents the number of failures until a target number of successes $k_j$, and that each $k_j$ is fixed and known. Under this parametrisation, the dispersion parameter is equal to 1.
 \green{Further, note that when the data model is binomial or negative-binomial, (\ref{eqn:Z_j|mu(.)}) also includes a size parameter, $k_{Zj}$, in which case we write $Z_j \sim \text{Bin}(\mu_{Zj}; k_{Zj})$ or $Z_j \sim \text{NB}(\mu_{Zj}; k_{Zj})$. However, for notational convenience, we suppress this notation for the remainder of this section. 
 }
...
\end{gather}
 ...
 \green{When the data model is binomial or negative-binomial, (\ref{eqn:new_model_Z}) is replaced with $Z_j \sim \text{Bin}(\mu_{Zj}; k_{Zj})$ or $Z_j \sim \text{NB}(\mu_{Zj}; k_{Zj})$, and if a `link' function is chosen that is appropriate for modelling probabilities, (\ref{eqn:new_model_g(mu)}) is replaced with 
\begin{gather}
 h(\vec{\mu}; \vec{k}) = \vec{\pi},\\
 f(\vec{\pi}) = \vec{Y},
\end{gather} 
where $h(\cdot; \cdot)$ and $f(\cdot)$ are discussed in greater detail in Appendix \ref{Appendix:Distributions with size parameters}.}
...
\subsubsection{Arbitrary prediction regions}
 ...
 Often, one does not wish to predict over a single BAU, but over regions spanning multiple BAUs\green{, $\tilde{S}_k$, $k = 1, \dots, N_P$, where $N_P$ is the number of prediction regions}. 
 These regions may overlap and may not coincide with entire BAUs: Our criterion for determining whether a region, $\tilde{S}_k$, contains a BAU is the same as that used for the spatial supports originally associated with the observations (see Section \ref{subsection:04-02:DataLayer}). 
 \green{That is, the indices of the BAUs associated with $\tilde{S}_k$ are $\tilde{c}_k \equiv \{i : A_i \cap \tilde{S}_k \neq \emptyset\}$, for $k = 1, \dots, N_P$.}
 We then define the set of prediction regions as $D^P \equiv \{\tilde{B}_k : k = 1, \dots, N_P\}$, where $\tilde{B}_k \equiv \cup_{i\in \tilde{c}_k} A_i$. %, and where $\tilde{c}_k$ is a non-empty subset of $\{1, \dots, N\}$ that gives the indices of the BAUs associated with the $k$th user-specified region, $\tilde{S}_k$.
...
Prediction over $D^P$ requires some form of aggregation across the associated BAUs. 
 \green{We aggregate at the mean scale, that is, we aggregate the mean process over the associated BAUs. Note that this does \textit{not} involve the aggregation of data.}
 Let $\vec{\mu}_P \equiv \{\mu(\tilde{B}_k) : k = 1, \dots, N_P\}$ be the mean process evaluated over the prediction regions. Just as $\vec{\mu}_Z$ was constructed from the BAU-level mean process $\vec{\mu}$ via the matrix $\vec{C}_Z$ given by (\ref{eqn:C_Z}), since each $\tilde{B}_k$ is a BAU or a union of BAUs, one can construct an $N_P \times N$ matrix 
\[
\vec{C}_P \equiv \left(\tilde{w}_{ik}\mathbb{I}(i \in \tilde{c}_k) : i = 1, \dots, N; k = 1, \dots, N_P\right),
\]
...
\begin{figure}[t!]
    ...
    \vspace{-10pt}    
    \caption{Simulated, point-referenced, Poisson data set used in the illustrative example of Section \ref{sec:03-01:Poisson}. (Left panel) True mean process, $\mu(\cdot)$, evaluated over the BAUs. (Right panel) Simulated Poisson data set.}
...
Next, an \class{SRE} object is constructed using \fct{SRE}, within which we specify the data model, the link function, and the parameterisation of $\cov{\vec{\eta}}{\vec{\eta}}$.  
 \green{In this example, we model counts using a Poisson data model, $Z_j \sim \text{Pois}(\mu_{Zj}, \psi)$, $j = 1, \dots, m$, and we use the log link function, $g(\cdot) = \log(\cdot)$. These choices are made by setting \code{response = "poisson"} and \code{link = "log"}.} 
...
The predictions of \green{$\vec{\mu}$} are reasonable given the data and the true process shown in Figure \ref{fig:Poisson_true_and_Z}. 
...
\begin{figure}[t!]
    ...
    \vspace{-20pt}  
    \caption{Predictions and prediction-interval widths using the data shown in Figure \ref{fig:Poisson_true_and_Z}. (Left panel) Prediction of the latent process, $Y(\cdot)$. (Centre-left panel) Width of the 90\% central prediction interval for $Y(\cdot)$. (Centre-right panel) Prediction of the mean process, $\mu(\cdot)$. (Right panel) Width of the 90\% central prediction interval for $\mu(\cdot)$.}
...
% This simulation procedure is illustrated in Figure \ref{fig:03-02-negative-binomial_data}. 
 ...
 \green{In the the first step of data simulation, we define a fine grid which will be used as the BAUs.
We construct the true probability process evaluated over the BAUs, $\vec{\pi}$, by passing a sum of trigonometric functions through the logistic function. 
We then construct the mean process evaluated over the BAUs, $\vec{\mu}$; as we are simulating negative-binomial data, this requires specification of known size parameters and, for simplicity, we set $k_i = 50$, $i = 1, \dots, N$, for each BAU. 
We then define the areal supports that will be used to generate data. 
 To get a handle on the fine-scale-variation parameter, we include both large-scale and fine-scale observations: That is, some data supports are large areal regions comprising many BAUs, while other data supports
coincide with the BAUs.  
 We then aggregate the mean process over the data supports to construct $\vec{\mu}_Z$, and simulate data using the aggregated mean process, $Z_j \sim \text{NB}(\mu_{Zj}, k_{Zj})$, $j = 1, \dots, m$. 
 This simulation procedure is illustrated in Figure \ref{fig:03-02-negative-binomial_data}.}
...
\begin{figure}[t!]
    ...
    \green{In this example, we have both large-scale and fine-scale observations: That is, some data supports are large areal regions comprising many BAUs, while other data supports coincide with the BAUs. The white regions correspond to areas that are unobserved.}
}   
...
\green{In this example, the data model is $Z_j \sim \text{NB}(\mu_{Zj}, k_{Zj})$, $j = 1, \dots, m$. We use the logit `link' function, whereby the latent process $Y(\cdot)$ is linked to the probability process, $\pi(\cdot)$, via $f(\pi(\cdot)) = Y(\cdot)$, where $f(\cdot) = \text{logit}(\cdot)$, and the mean process is linked to the probability process via $h(\mu(\cdot); k(\cdot)) = \pi(\cdot)$, where $h(\cdot, \cdot)$ and $k(\cdot)$ are defined in Appendix \ref{Appendix:Distributions with size parameters}.} 
...
%In many non-Gaussian applications, it is natural to sum the mean process over the BAUs; this instruction is given by setting \code{normalise\_wts = FALSE}. 
 For binomial and negative-binomial data, known size parameters must be provided: In general, we need these size parameters for every observed BAU\footnote{\red{Noel crossed out ``observed BAU'' and replaced it with ``observation''.} \green{I disagree, since our model requires the construction of $\vec{\mu}$ (actually, a subset of $\vec{\mu}$ over only those BAUs associated with an observation; perhaps I need to describe this?).}}. 
 In the special case that each observation is associated with exactly one BAU (e.g., point-referenced data, or areal data where the BAUs and observation supports coincide), the user can provide the size parameters either through the observations or through the BAUs. When some observations are associated with multiple BAUs, the user must provide the size parameters at the BAU level (at least for all observed BAUs). 
 \green{Then, the size parameters at the data-support level are obtained by simply summing the individual BAU-level size parameters, which is reasonable since \pkg{FRK} v2 enforces $\vec{C}_Z$ and $\vec{C}_P$ to represent a simple, un-weighted summation of the mean process when the data model is binomial or negative-binomial (see Section \ref{SEC:Methodology}).}\footnote{
 \red{Noel, does this answer your question ``So how are individual BAU size parameters used to get a size parameter for areal-level supports?''}}
...
\begin{figure}[t!]
    ...
    \vspace{-20pt}
    \caption{Prediction and prediction-interval width for the simulated negative-binomial areal data set shown in the right panel of Figure \ref{fig:03-02-negative-binomial_data}. (Left panel) Prediction of the probability process, $\pi(\cdot)$. (Centre-left panel) Width of the 90\% central prediction interval for $\pi(\cdot)$. (Centre-right panel) Prediction of the mean process, $\mu(\cdot)$. (Right panel) Width of the 90\% central prediction interval for $\mu(\cdot)$.
}   
  \label{fig:03-02-negative-binomial}
\end{figure}

Figure \ref{fig:03-02-negative-binomial} shows the predictions and prediction-interval widths for both the mean process, $\mu(\cdot)$, and the probability process, $\pi(\cdot)$, at the BAU level. 
...
The prediction-interval width for $\mu(\cdot)$ is roughly proportional to its prediction. 
In contrast, the prediction-interval width for $\pi(\cdot)$ is low when the prediction is near 0 or 1, and it increases when the prediction is near 0.5: This is expected from properties of the negative-binomial distribution. 
...
To emphasise that the prediction polygons are unrelated to the BAUs and data supports \red{(but don't we use BAUs to work with discretised polygons? Perhaps I should say: ``... that the shapes of the prediction polygons are unrelated to those of the BAUs and data supports'')}, we demonstrate prediction over a handful of irregularly-shaped areas (defined as a \class{SpatialPolygons} object).  
\begin{Code}
R> pred <- predict(S, newdata = arbitrary_polygons)
\end{Code}
Figure \ref{fig:03-02-negative-binomial_polygons} shows the predictions and prediction-interval widths for $\mu(\cdot)$ over the irregularly-shaped areas. 

\begin{figure}[t!]
    \centering
    \includegraphics[width = \linewidth]{img/Negbinom_sim_arbitrary_polygon_predictions.png}
	\vspace{-20pt}    
    \caption{Prediction (left panel) and prediction-interval widths (right) for the aggregated mean, $\vec{\mu}_P$, when predicting over arbitrary polygons in the example of Section \ref{sec:03-03:negative-binomial}. Note that these polygons have equal area.}   
...
\begin{table}[t!]
    \centering
    \caption{Diagnostics comparing the predictive performance when using a range of basis-function resolutions with point-referenced count data. The diagnostics are the root-mean-squared prediction error (RMPSE), the continuous ranked probability score (CRPS), and the empirical coverage (Cvg90) and interval score (IS90) resulting from a central prediction interval with a nominal coverage of 90\% (see Appendix \ref{app:ScoringRules} for further details). The diagnostics are with regard to prediction of the BAU-level mean, $\vec{\mu}$, at all unobserved locations and they are averaged.
...
\pkg{FRK} v2 also caters for non-Gaussian, point-referenced and areally-referenced, \textit{spatio-temporal} data. \red{(Noel suggests ``point-referenced and areally-referenced'' should be ``BAU-level and areal-level''; I disagree, because the data itself is originally point-referenced, irrespective of the fact that we associate it to BAUs.)} 
...
\red{Need to re-run the study so that the labels get changed to $s_1$ and $s_2$ (might actually have the data stored on the HPC already; if so, just use that).}
...
% In a binary setting, the task of assessing prediction uncertainty quantification is difficult, as prediction intervals (based on the probability samples) are in the range $(0, 1)$, and so they will \textit{never} contain the validation data, which take a value in the set $\{0, 1\}$.
The packages used in this study can provide prediction standard errors associated with the probability process, $\pi(\cdot)$. 
However, the underlying distribution of the probability process is unidentifiable \red{?}, as the predictive distribution, ${Z^*} \mid \vec{Z}$, for some validation datum ${Z^*}$, depends only on the posterior expectation of the probability parameter at the corresponding location, $\ENoLR{\pi^* \mid \vec{Z}}$.
For this reason, we do not attempt to validate the prediction intervals, and instead focus our efforts on predictive accuracy. \red{Noel doesn't follow this paragraph. Could explain it better if we had a formula for the binomial distribution. }
...
\begin{figure}
    ...
    \includegraphics[width = \linewidth]{img/Am_data_and_blocks.png}
        \vspace{-20pt}
    \caption{Americium soil data. The `$\times$' denotes Ground Zero (GZ), where the devices were detonated. (Left panel) Am concentrations on the original scale. (Centre panel) Am concentrations on the log scale. (Right panel) Blocking schemes: Scheme 1 (red), centred away from GZ, and Scheme 2 (blue), centred on GZ; for context, the black points show the measurement locations. \red{In the right-panel, red and blue has nothing to do with the colour scale used in the left and centre-left panels; possibly use another aesthetic, or completely different colours (e.g., purple and green)?}}   
  \label{fig:Am_data}
\end{figure}
...
%    \caption{Prediction and root mean squared prediction error (RMSPE) against block size for the two blocking schemes. (Left panel) Block-predictions of Am concentrations against block size, $|B|^{1/2}$.  (Right panel) RMSPE of block predictions of Am concentrations against block size $|B|^{1/2}$. In both plots, the red line corresponds to Scheme 1 and the blue line corresponds to Scheme 2.}   
    \caption{Block-predictions (left panel) and root mean squared prediction error (right panel) of Am concentrations against block size, $|B|^{1/2}$. Both quantities are in units of $10^3$ counts per minute. In both panels, the red line corresponds to Scheme 1 and the blue line corresponds to Scheme 2.}   
...
In this example, we use the SA2-region (and some SA1-region) data for training the model, and we use the SA1 regions 
%(no data included) 
...
%SA2 region
data polygon as the sum (rather than the average) of the mean process over the SA1 regions.
\red{Need to give the data (binomial) model and link function in the text (as a formula). }
\begin{Code}
...
\end{Code}
Since the SA1 regions are our BAUs, we can predict both the probability process and the mean process over the SA1 regions: We focus on the probability process, which is independent of the size parameter. \red{(Give the data model on the previous page, where the prob process and the size parameter are defined.)}
...
\end{Code}
Figure \ref{fig:SA3_predictions} shows the SA3-region predictions and associated prediction-interval widths of the mean process. Again, this graphic was generated using \fct{plot}. 
...
%The data considered in this analysis consisted of 1,748,360 crimes. 
 We considered only crimes labelled as assault or battery; there were roughly 1.75 million crimes in total. 
 Note that the default behaviour of all releases of \pkg{FRK} is to bin data falling into the same BAU; in this example, the final number of observations post-binning is 1444. \red{Individual-level data and aggregated-level data} 
...
Then, we initialise and fit the \class{SRE} object using \fct{FRK}, setting \code{response = "poisson"} and \code{link = "log"}. \red{(refer back to the Poisson model that we will define earlier in the paper.)}
 Each entry in our data set provides the location and time at which a given crime occurred. It also contains a column of ones called \code{"number\_of\_crimes"}; this will be used for counting the number of crimes when binning. 
 By default, \fct{SRE}, which is called internally within \fct{FRK}, bins and then averages data falling into the same BAU. 
 We wish to model the total number of crimes in a given BAU; hence, we wish to sum the binned data instead of taking an average. To do so, we pass the name of the response variable (\code{"number\_of\_crimes"}) via the argument \code{sum\_variables}.
...
\red{(Refer to the data models introduced earlier in the paper.)} 
