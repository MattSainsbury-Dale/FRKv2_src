...
\red{We now discuss spatial prediction and uncertainty quantification of the predictions. 
...
\end{equation}
where each of the $n_{\text{MC}}$ columns of the matrix $\hatt{\vec{A}}[A]$ is the \red{corresponding} estimated posterior mode of $\vec{\alpha}$, and each of the $n_{\text{MC}}$ columns of the matrix $\vec{U}$ is a draw from $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$.  
...
\subsection{Example: Non-Gaussian, point-referenced spatial data}\label{sec:03-01:Poisson}
 ...
 The true mean process evaluated over the BAUs, $\vec{\mu}$, is also shown in Figure \ref{fig:Poisson_true_and_Z}\red{: It was constructed by first simulating $\vec{Y}$ from a Gaussian process with with an exponential covariance function with variance parameter $\sigma^2 = 0.1$ and length-scale parameter $\tau = 0.2$, and then passing $\vec{Y}$ through the exponential function.}
...
%The training set consists of 105,569 observations, while the test set consists of 42,740 observations.
 ...
 The results show that the increased number of basis functions from \pkg{FRK} v2 significantly improved the diagnostic scores over \pkg{FRK} v1, so that the results for \pkg{FRK} v2 are now comparable to the \red{heretofore-preferred} spatial-prediction method MRA.  
