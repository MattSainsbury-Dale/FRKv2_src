...
% Couldn't find a solution... Just using \clearpage at problematic headings, for now. The sections that I have put a \clearpage are:
% - 2.1: process layer
...
\Abstract{
 Non-Gaussian spatial and spatial-temporal data are becoming increasingly prevalent, and their analysis is needed in a variety of disciplines, such as those involving small-area demographics or global remote sensing instruments. 
...
\citet{Bradley_2018_computationally_efficient_multivariate_ST_models_for_high-dimensional_count-valued_data}, \citet{Bradley_2019_ST_models_for_big_multinomial_data_using_conditional_multivariate_logit_beta_distribution}, and \citet{Zhang_2020_spatio-temporal_Arctic_sea_ice}. 
 ...
 A project aimed at facilitating spatial statistical modelling using \pkg{INLA} is the \pkg{inlabru} package \citep{Bachl_2019_inlabru}; at the time of writing,  spatio-temporal modelling was not implemented in \pkg{inlabru} \red{(CHECK)}.
 ...
 The main purpose of this article is to present a major upgrade to \pkg{FRK} that allows one to cater for many distributions within the exponential family using the spatial GLMM framework; we henceforth refer to it as \pkg{FRK} v2 and the original version as \pkg{FRK} v1. 
 \pkg{FRK} v2 provides a unifying framework that handles large, spatial and spatio-temporal non-Gaussian data, and it can seamlessly ingest point-referenced and area-referenced data to solve spatial change-of-support problems. %facilitates change-of-support over target supports that may be smaller or larger than the source supports (i.e., it caters for statistical down-scaling and up-scaling). 
...
\textit{data layer}, we use a conditionally independent exponential-family model for each element of the data vector; see Section \ref{subsection:04-02:DataLayer}. 
 \green{In Section \ref{sec:Distributions with size parameters}, we consider allowed distributions that have an assumed-known `size' parameter, %associated with each datum, 
...
Accounting for only the scales of spatial variation with just $\vec{t}(\cdot)^\tp \vec{\alpha}$ and $v(\cdot)$ can result in overly optimistic predictions; this problem is alleviated by also including a fine-scale-variation random process, $\xi(\cdot)$. 
...
BAUs are a key element of \pkg{FRK} v1/v2, as they provide a framework that allows one to easily consider point-referenced and areal data simultaneously, and they facilitate solutions to spatial change-of-support problems. 
 Let $Y(A_i)$ denote a representative value of $\{Y(\vec{s}) : \vec{s} \in A_i\}$, where commonly the spatial integral or the spatial average over $A_i$ is chosen.  
...
\end{equation}
 where $\vec{T}$ and $\vec{S}$ are known design matrices constructed from $\vec{t}(\cdot)$ and $\vec{\phi}(\cdot)$ respectively, and $\vec{\xi}$ is a vector associated with the fine-scale process which, like $\vec{\eta}$, is treated as a random effect.

As in \pkg{FRK} v1, the elements of $\vec{\xi}$ are modelled as independent and identically distributed (i.i.d.) Gaussian random variables with mean zero and variance $\sigma^2_\xi$, and $\vec{\eta}$ is modelled as a mean-zero multivariate-Gaussian random variable with covariance matrix $\cov{\vec{\eta}}{\vec{\eta}}$. 
...
% Figure \ref{fig:BAU_intuition} shows a simple example illustrating $D$ is discretised into $N = 12$ BAUs, and how $B_1$, $B_2$, and $B_3$ are defined from $m=3$ observations, resulting in $D^O \equiv \{B_1, B_2, B_3\}$.
 ...
 We denote the vector of $m$ observations (the data vector) as $\vec{Z} \equiv \left(Z_1, \dots, Z_m\right)^\tp$.
   \green{Each datum is originally associated with a spatial support, $R_j$, $j = 1, \dots, m$, which we associate to one or more BAUs. 
...
Define the \green{conditional mean of the data as $\vec{\mu}_Z \equiv \left(\E{Z_1 \mid \vec{\mu}}, \dots, \E{Z_m \mid \vec{\mu}}\right)^\tp$}. 
...
In \pkg{FRK} v2, the weights $w_{ij}$ may be controlled through the \code{wts} field of the BAUs object and the argument \mbox{\code{normalise\_wts}}. 
 ...
 By default (and implicit in \pkg{FRK} v1), each $v_i$ is set to 1. 
  The argument \mbox{\code{normalise\_wts}} controls whether $\vec{C}_Z$ corresponds to a weighted sum or a weighted average. If set to \code{FALSE}, then $w_{ij} = v_i$ for all $j$ (weighted sum); if set to \mbox{\code{TRUE}} (default and implicit in \pkg{FRK} v1), then the $\{w_{ij}\}$ are normalised so that each row of $\vec{C}_Z$ sums to 1 (weighted average).  
...
where EF corresponds to a probability distribution in the exponential family with dispersion parameter $\psi$ and, for generic random quantities $A$ and $B$, $[A \mid B]$ denotes the probability distribution of $A$ given $B$. 
 We assume that $\psi$ is spatially invariant; with the parameterisations assumed by \pkg{FRK} v2, $\psi = 1$ for some distributions in the exponential family (e.g., binomial, negative-binomial, and Poisson distributions).

\green{Together, (\ref{eqn:04-01:mu_Z})} and (\ref{eqn:Z_j|mu(.)}) imply that a given observation depends only on the value of the mean process at the corresponding observation support, rather than on means elsewhere in the domain. 
Further, we assume that all observations are conditionally independent given the latent spatial process, \green{and that they are all from the same exponential family member}: Specifically, 
\[
...
the vector of size parameters associated with the data, $\vec{k}_Z \equiv (k_{Z_1}, \dots, k_{Zm})^\tp$, the parametrisation used in \pkg{FRK} v2 assumes that: $Z_j$ represents the number of `successes' from $k_{Z_j}$ trials (binomial data model) or the number of failures before $k_{Z_j}$ successes (negative-binomial data model).}
...
\green{
 ...
 In the special case that each observation is associated with exactly one BAU (e.g., point-referenced data), %, or areal data where the BAUs and observation supports coincide), 
 the user can provide the size parameters either through the observations or through the BAUs. 
 When one or more observations are associated with multiple BAUs, the user must provide the size parameters at the BAU level:  % (at least for all observed BAUs).
 In this case, if the size parameter at the data level, $\vec{k}_Z$, is not provided explicitly, it is obtained by summing the BAU-level size parameters.\footnote{\red{Andrew: You said this is a contradiction, can we discuss.}} %Specifically, $$\vec{k_Z} = \vec{C}_Z \vec{k}.$$
 }

\green{In most applications considering binomial or negative-binomial data models, 
%  that is, when $Z_j \sim \text{Bin}(\mu_{Z_j}; k_{Z_j})$ or $Z_j \sim \text{NB}(\mu_{Z_j}; k_{Z_j})$, $j = 1, \dots, m$, 
 the conditional mean of an observation is treated as a simple aggregate of the underlying mean process. Therefore, with these distributions, \pkg{FRK} v2 requires that the matrix $\vec{C}_Z$ in (\ref{eqn:C_Z}) is constructed with the relative weights \mbox{$\{v_i = 1$ : $i = 1, \dots, N\}$} and with \mbox{\code{normalise\_wts = FALSE}}. Then, the mapping from the BAU-level mean, $\vec{\mu}$, to the data-level mean, $\vec{\mu}_Z$, in (\ref{eqn:04-01:mu_Z}) is a simple, un-weighted summation over the BAUs.}
 


% \green{When the response distribution is binomial or negative-binomial, (\ref{eqn:new_model_Z}) 
%% is replaced with $Z_j \sim \text{Bin}(\mu_{Z_j}; k_{Z_j})$ or $Z_j \sim \text{NB}(\mu_{Z_j}; k_{Z_j})$, 
...
Recall that the Laplace approximation implies that, approximately, $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$; since $\vec{Y}$ is a linear function of $\vec{u}$, approximate inference on $Y(\cdot)$ can be done using well-known formulas. 
  ...
  In \pkg{FRK} v2, we use a Monte Carlo (MC) approach to inference of $\vec{u} \equiv (\vec{\eta}^\tp, \vec{\xi}^\tp)^\tp$.

 Recall that $\vec{Y} = \vec{T}\vec{\alpha} + \vec{S}\vec{\eta} + \vec{\xi}$, which can be rewritten as $\vec{Y} = \vec{T}\vec{\alpha} + [\vec{S} \; \vec{I}] \,\vec{u}$. 
 We thus define $\vec{Y}_{\!\text{MC}}$, an $N \times n_{\text{MC}}$ matrix whose columns are $n_{\text{MC}}$ MC samples of $\vec{Y} \mid \vec{Z}, \vec{\theta}$, as 
...
where every column of the matrix $\vec{A}$ is the estimated posterior mode of $\vec{\alpha}$, and each of the $n_{\text{MC}}$ columns of the matrix $\vec{U}$ are draws from  $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$.  
 We obtain MC samples of the $N$-dimensional vector $\vec{\mu}$ from $[\vec{\mu} \mid \vec{Z}, \vec{\theta}]$ via $\vec{M} \equiv g^{-1}(\vec{Y}_{\!\text{MC}})$, where $g^{-1}(\cdot)$ is applied element-wise. 
  \green{\pkg{FRK} v2 also allows prediction of data over all $N$ BAUs, $\vec{Z}^* \equiv (Z^*_1, \dots, Z^*_N)^\tp$. 
  We assume that these data are from the same exponential family model as that of the original data, $\vec{Z}$. %: That is, $Z^*_i \sim \text{EF}(\mu_i, \psi)$, $i = 1, \dots, N$.  
 ...
 Note that \pkg{FRK} v2 provides the user with $\vec{M}$, which in turn can be used to predict data from a distribution that is different from that of the original data if needed.}
...
Prediction over $D^P$ requires some form of aggregation across the associated BAUs. 
 \green{In \pkg{FRK} v2, we aggregate the mean process over the associated BAUs. We stress that this is different from aggregation of data (which would lead to a different model for dealing with change-of-support).}
 Let $\vec{\mu}_P \equiv \{\mu(\tilde{B}_k) : k = 1, \dots, N_P\}$ be the mean process evaluated over the prediction regions. Just as $\vec{\mu}_Z$ was constructed from the BAU-level mean process $\vec{\mu}$ via the matrix $\vec{C}_Z$ given by (\ref{eqn:C_Z}), since each $\tilde{B}_k$ is a BAU or a union of BAUs, one can construct an $N_P \times N$ matrix 
\[
\vec{C}_P \equiv \left(\tilde{w}_{ik}\mathbb{I}(i \in \tilde{c}_k) : i = 1, \dots, N; k = 1, \dots, N_P\right),
\]
...
As in Section \ref{subsection:04-02:DataLayer}, \green{the relative weights, $\{v_i: i = 1, \dots, N\}$, such that $\tilde{w}_{ik} \propto v_i$, are controlled by the \code{wts} field of the BAU object, and the argument \mbox{\code{normalise\_wts}} is used to control whether $\vec{C}_P$ represents a weighted sum or a weighted average. 
 For consistency between the model fitting and prediction stages, \pkg{FRK} v2 uses the same relative weights, $\{v_i: i = 1, \dots, N\}$, and the same setting of \code{normalise\_wts}, in construction of both $\vec{C}_Z$ and $\vec{C}_P$.}


 MC samples of $\vec{\mu}_P \mid \vec{Z}, \vec{\theta}$ are constructed  via $\vec{M}_P \equiv \vec{C}_P \vec{M}$, where recall that $\vec{M}$ consists of MC samples from $[\vec{\mu} \mid \vec{Z}, \vec{\theta}]$. Predictions and uncertainty quantification of the predictions can then be computed straightforwardly from $\vec{M}_P$. 
   \green{\pkg{FRK} v2 also allows inference on data over aggregations of BAUs, $\vec{Z}^*_P \equiv (Z^*_{P, 1}, \dots, Z^*_{P, N_P})^\tp$. 
   We assume that these are from the same exponential family model as that of the original data, $\vec{Z}$. %: That is, $Z^*_{Pk} \sim \text{EF}(\mu(\tilde{B}_k), \psi)$, $k = 1, \dots, N_P$.  
 ...
 Note that \pkg{FRK} v2 provides the user with $\vec{M}_P$, which in turn can be used to predict data from a distribution that is different from that of the original data if needed.}
...
This form for $\vec{Q}$ leads to significant computational savings. 
  For the random coefficients associated with the temporal basis functions, \pkg{FRK} v2 uses a \green{first-order autoregressive} model. 
...
\green{In these situations, \pkg{FRK} v2 allows each spatial BAU to be associated with its own fine-scale variance parameter. 
...
We now demonstrate the new features in \pkg{FRK} v2, an overview of which is presented in Table \ref{tab:new_arguments_in_functions}. 
 ...
 Finally, in Section \ref{sec:3:increased_resolution}, we show the potential improvement in predictive performance of \pkg{FRK} v2 over \pkg{FRK} v1 when the data are Gaussian, thanks to an increase in the maximum number of basis functions allowed in \pkg{FRK} v2. 
...
Next, an \class{SRE} object is constructed using \fct{SRE}, within which we specify the data model, the link function, and the parameterisation of $\cov{\vec{\eta}}{\vec{\eta}}$.  
 \green{In this example, we model counts using a Poisson data model, $Z_j \sim \text{Pois}(\mu_{Z_j})$, $j = 1, \dots, m$, and we use the log link function, $g(\cdot) = \log(\cdot)$. These choices are made by setting \code{response = "poisson"} and \code{link = "log"}.} 
...
The \code{percentiles} argument allows the computation of percentiles of the predictive distributions and hence of prediction intervals; if left unspecified, the 5th and 95th percentiles are returned: 
\begin{Code}
...
The predictions of \green{$\vec{\mu}$} are reasonable given the data and the true values shown in Figure \ref{fig:Poisson_true_and_Z}. 
...
% This simulation procedure is illustrated in Figure \ref{fig:03-02-negative-binomial_data}. 
 ...
 In the the first step, we define a fine grid over which to construct the BAUs.
We generate the true probability process evaluated over the BAUs, $\vec{\pi}$, by passing a sum of trigonometric functions through the logistic function; $\vec{\pi}$ is shown in the left panel of Figure \ref{fig:03-02-negative-binomial_data}. 
...
That is, some data supports are large areal regions comprising many BAUs, while others coincide with the BAUs.  
 ...
 Finally, we simulate negative-binomial data, $Z_j \sim \text{NB}(\mu_{Z_j}, k_{Z_j})$, $j = 1, \dots, m$,  shown in the right panel of Figure \ref{fig:03-02-negative-binomial_data}.}
...
\green{In this example, the data model is negative-binomial, that is, $Z_j \sim \text{NB}(\mu_{Z_j}, k_{Z_j})$, $j = 1, \dots, m$, and we use the logit `link' function (see Appendix \ref{Appendix:Distributions with size parameters}).} 
...
In contrast, the prediction-interval width for $\vec{\pi}$ is low when the prediction is near 0 or 1, and it increases when the prediction is near 0.5: This is expected from properties of the negative-binomial distribution. 
 Uncertainty for both quantities is lower over areas in which we have fine-scale data. 
 The mean empirical coverage from the 90\% prediction intervals was 90.9\%, which is almost nominal.

\green{To illustrate that user-specifedprediction regions, $\tilde{R}_k$, $k = 1, \dots, N_P$, are arbitrary, we demonstrate prediction over a handful of irregularly-shaped areas (defined as a \class{SpatialPolygons} object).}
\begin{Code}
R> pred <- predict(S, newdata = arbitrary_polygons)
\end{Code}
Figure \ref{fig:03-02-negative-binomial_polygons} shows the predictions and prediction-interval widths, respectively, for the mean process \green{evaluated over the irregularly-shaped areas, $\vec{\mu}_P$}. 
...
The efficiency of \pkg{TMB} and our use of $\vec{Q}$ instead of $\vec{K}$, means that \pkg{FRK} v2 is now better equipped than \pkg{FRK} v1 to use a large number of basis functions. 
This is important, as the predictive performance of \pkg{FRK} is often determined by the number of basis functions, as can be seen from the results of the following experiments. 
...
In our first experiment, we repeated the analysis in Section \ref{sec:03-01:Poisson} using one, two, and three resolutions of basis functions; Table \ref{tab:03-02:PoissonScoringRules} shows the results for each run. 
 ...
 This result is in large part due to the presence of the fine-scale random variation term, $\xi(\cdot)$, in (\ref{eqn:04-01:Y(s)}). 
\begin{table}[t!]
    \centering
...
In our second experiment, we re-ran the analysis for the comparative study published in \cite{Heaton_2019_comparative_study}.
...
\green{Then, as the data provided by the MODIS instrument is continuous (measuring spectral radiances in units of $\text{W/m}^2/\si{\um}/\text{st}$), we applied a reasonable threshold to obtain a binary version of the data: Specifically, pixels with spectral radiance greater than 7,000 were labelled `Cloud', and the remaining pixels were labelled `No-cloud'. 
 In this example, the data model is $Z_j \sim \text{Bin}(\mu_{Z_j}, 1)$, $j = 1, \dots, m$, where $\mu_{Z_j}$ represents the probability of `Cloud' over the pixel $R_j$. 
}  
...
Modelling for this problem is done by setting \code{response = "Gaussian"} and \code{link = "log"}. 
 ...
 \green{In summary, the model is $Z_j \sim \text{Gau}(\mu_{Z_j}, 1)$, $j = 1, \dots, m$, with a log link function, $g(\cdot) = \log(\cdot)$.}
...
\includegraphics[width = \linewidth]{img/Sydney_training_data_mixed.png}
    ...
    (Right panel) The observed proportion of families in poverty computed by dividing the number of families in poverty by the total number of families, \green{that is, $Z_j / k_{Z_j}$, $j = 1, \dots, m$.} % at the SA2 level, 
...
In this example, we use the SA2-region (and some SA1-region) data for training the model, and we use the SA1 regions (no data included) as the BAUs; these are passed as \class{SpatialPolygonsDataFrame} objects to \fct{FRK}. 
 \green{Our data model is $Z_j \sim \text{Bin}(\mu_{Z_j}, k_{Z_j})$, $j = 1, \dots, m$, and we use the logit `link' function (see Appendix \ref{Appendix:Distributions with size parameters}).}
...
Then, we initialise and fit the \class{SRE} object using \fct{FRK}, setting \code{response = "poisson"} and \code{link = "log"} \green{in order to use a Poisson data model, $Z_j \sim \text{Pois}(\mu_{Z_j})$, $j = 1, \dots, m$, and a log link function, $g(\cdot) = \log(\cdot)$.} 
...
\section{Link functions for distributions with size parameters}\label{Appendix:Distributions with size parameters}
 ...
 \green{Two distributions considered in this framework, namely, the binomial distribution and the negative-binomial distribution, have an assumed-known `size' parameter and a `probability of success'
parameter. 
%Given data, $\vec{Z} \equiv (Z_1, \dots, Z_m)$, and assumed-known `size' parameters, $\vec{k}_Z \equiv (k_{Z_1}, \dots, k_{Zm})^\tp$, the parametrisation used in \pkg{FRK} v2 assumes that: For binomial data models, $Z_j$ represents the number of `successes' from $k_{Z_j}$ trials; for negative-binomial data models, $Z_j$ represents the number of failures before $k_{Z_j}$ successes. 
