...
% This approach to mapping data supports to BAUs is slightly different from \pkg{FRK} v1, which assumed that a spatial support contains a BAU if and only if the BAU centroid lies within the spatial support. This modification to \pkg{FRK} v2 \red{also} caters for non-convex BAUs, such as those used in Section \ref{sec:spatialCOS}, where the centroid of a given BAU may lie outside of the BAU boundary. 
 Figure \ref{fig:BAU_intuition} shows a pedagogical example with $m = 3$ observations illustrating the relationship between the continuous domain $D$, the BAUs \red{$\{A_1, \dots, A_{12}\}$} that make up \red{$D^G$}, the original spatial supports $\{R_1, R_2, R_3\}$, and the observation supports $\{B_1, B_2, B_3\}$ \red{that make up $D^O$}. %in \pkg{FRK} v2.
...
(Centre-left panel) The spatial domain discretised into $N = 12$ BAUs\red{, $D^G$}. 
...
There are three principal quantities that could be of interest to the user, namely the latent process $Y(\cdot)$ and mean process $\mu(\cdot)$ in (\ref{eqn:mu_linked_to_Y}), and data at unobserved locations. 
...
Recall that the Laplace approximation implies that, approximately, $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$; since $\vec{Y}$ is a linear function of $\vec{u}$, approximate inference on $Y(\cdot)$ can be carried out using well-known formulas. 
  ...
  In \pkg{FRK} v2, we use a Monte Carlo (MC) approach to inference on $\vec{u} \equiv (\vec{\eta}^\tp, \vec{\xi}^\tp)^\tp$.\footnote{\red{Back and forth between the process versions, $Y(\cdot)$ and $\mu(\cdot)$, and the vectorised versions, $\vec{Y}$ and $\vec{\mu}$}}
...
\vec{Y}_{\!\text{MC}}
\equiv \vec{T}\hat{\vec{A}} + [\vec{S} \; \vec{I}] \,\vec{U},
\end{equation}
where each of the $n_{\text{MC}}$ columns of the matrix $\hat{\vec{A}}$\footnote{\red{need to fix the hat here and above}} is the \red{corresponding} estimated posterior mode of $\vec{\alpha}$, and each of the $n_{\text{MC}}$ columns of the matrix $\vec{U}$ is a draw from $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$.  
...
\subsection{Distributions with size parameters}\label{sec:Distributions with size parameters}
 ...
 \red{Note that some sources used different parameterisations for the negative-binomial distribution: The parameterisation used in \pkg{FRK} v2 is the same as that used in the \proglang{R} base-package \pkg{stats} \citep{Rcoreteam_2021}.}
...
\mbox{$\vec{k} \equiv (k_i: 1, \dots, N)^\tp$} 
 ...
 We define the probability process evaluated over the observation supports, 
% $\{B_j : j = 1, \dots, m\}$,
 \red{$D^O$}, 
 as $\vec{\pi}_Z \equiv (\pi_{Z_j} : j = 1, \dots, m)^\tp$, where
\begin{equation}
...
%\end{equation}
 %% Vector version (avoids problems associated with subscripting)
  Now define the vector of size parameters associated with the prediction regions, 
%  $\tilde{B}_l$, $l = 1, \dots, N_P$, 
 \red{$D^P$},
  as $\vec{k}_P \equiv (\sum_{i \in \tilde{c}_l} k_i : l = 1, \dots, N_P)^\tp$, where recall that $\tilde{c}_l$ denotes the indices of the BAUs associated with the prediction region $\tilde{B}_l \red{\in D^P}$: Then the probability process evaluated over 
...
In this section, we illustrate \pkg{FRK} v2 on simulated negative-binomial, areally-referenced, spatial data, as well as its use in predicting over areas with large spatial supports. 
  In what follows, $\text{NB}(k, \pi)$ refers to a distribution on the non-negative integers with PMF $\{p_z: z = 0, 1, \dots\}$, namely \mbox{$p_z = {{z + k - 1}\choose z}\pi^k(1-\pi)^z$}, for $\pi \in (0, 1)$ and $k \in \{0, 1, \dots\}$, \red{where $z$ represents the number of ``failures'' before a target number of ``successes'', $k$}. 
...
We then construct the mean process, $\vec{\mu}$, evaluated over the BAUs, which is shown in the centre-left panel of Figure \ref{fig:03-02-negative-binomial_data}; as we are simulating negative-binomial data, this requires specification of assumed-known size parameters \red{representing the target number of successes}, and, for simplicity, we set $k_i = 50$, $i = 1, \dots, N$.  
 ...
 Finally, we simulate negative-binomial data, \mbox{$Z_j \mid \vec{\mu}, \vec{k}_Z \sim \text{NB}(k_{Z_j}, \pi_{Z_j})$}, $j = 1, \dots, m$, \red{which represent the number of failures before the target number of successes, $k_{Z_j}$, is reached}; these data are shown in the right panel of Figure \ref{fig:03-02-negative-binomial_data}. 
...
\end{figure}
 ...
 In this example, the data model is negative-binomial, that is, $Z_j \mid \vec{\mu}, \vec{k}_Z \sim \text{NB}(\pi_{Z_j}, k_{Z_j})$, $j = 1, \dots, m$, and we use the logit link function, \red{$f(\pi_i) = \log(\frac{\pi_i}{1 - \pi_i})$} in (\ref{eqn:pi_i_equals_f_Y_i}). 
 Since some observations are associated with multiple BAUs, we must provide the BAU-level size parameters (see Section \ref{sec:Distributions with size parameters}) \red{in the \code{k\_BAU} field of the BAU object. In the following code, \code{BAUs} is a fine grid (as described above) and is stored as a \code{SpatialPixelsDataFrame}.}
\begin{Code}
R> BAUs$k_BAU <- 50
\end{Code} 
...
To illustrate that user-specifed prediction regions, $\tilde{R}_l$, $l = 1, \dots, N_P$, are arbitrary, we demonstrate prediction over a handful of irregularly-shaped polygons. % (defined as a \class{SpatialPolygons} object).

\begin{Code}
R> pred <- predict(S, newdata = arbitrary_polygons)
\end{Code}

\red{Need to be careful with which prediction regions I am referring to!}


Figure \ref{fig:03-02-negative-binomial_polygons} shows the predictions and prediction-interval widths, respectively, for the probability process, $\vec{\pi}_P$, and the mean process, $\vec{\mu}_P$, when predicting over arbitrary prediction regions, $\tilde{R}_l$, $l = 1, \dots, N_P$. 
...
This is important, as the predictive performance of fixed rank kriging is often determined by the number of basis functions, as can be seen from the results of the following experiments. 
...
%The training set consists of 105,569 observations, while the test set consists of 42,740 observations.
 ...
 The results show that the increased number of basis functions from \pkg{FRK} v2 significantly improved the diagnostic scores over \pkg{FRK} v1, so that the results for \pkg{FRK} v2 are now comparable to the heretofore-preferred spatial-prediction method MRA.  
...
\end{table}
 ...
 To assess predictive accuracy, we compared the predictions from all models using the Brier score \citep[e.g., ][Sec.~3]{Gneiting_2007_scoring_rules}, and the area under the receiver operating characteristic (ROC) curve (AUC) \red{(REF)}.
...
As in \cite{Paul_Cressie_2011_lognormal_kriging_block_prediction}, we use a piece-wise linear trend in the `distance from GZ' as a fixed effect: Specifically, the log of the observations within a distance of 30.48m (100 ft) from GZ are assumed to follow a different trend to those observations beyond 30.48m from GZ, making up two regimes depending on distance from GZ. 
 \red{In \pkg{FRK} v1/v2, covariates are given at the BAU level with the BAU object; hence, in this example, we must first construct the BAUs, and we do this using the helper function \fct{auto\_BAUs}. 
 In the previous examples, which did not contain covariates, \fct{auto\_BAUs} was called internally within the function \fct{FRK}.}

\begin{Code}
R> BAUs <- auto_BAUs(manifold = plane(), type = "grid", data = Am_data)
\end{Code} 
 
 The following code constructs the BAU-level covariates that are needed to fit this piece-wise linear trend: \code{BAUs\$x1} and \code{BAUs\$x3} are indicator variables used to model the intercepts in each regime, and \code{BAUs\$x2} and \code{BAUs\$x4} are used to model the slopes of the trend in each regime. 
...
Spatial statistical modelling for this problem is done by setting \code{response = "Gaussian"} and \code{link = "log"} when calling \fct{FRK}. 
 ...
 Note that in the code below, we suppress the global intercept since we model the region-specific intercepts separately through \code{BAUs$x1} and \code{BAUs$x3}. 
\begin{Code}
R> Am_data$std <- 1 
...
R> SA1s$k_BAU <- SA1s$number_of_families
 ...
 Our data model is $Z_j \mid \vec{\mu}, \vec{k}_Z \sim \text{Bin}(k_{Z_j}, \pi_{Z_j})$, for $j = 1, \dots, m$, and we use the logit link function, $f(\pi_i) = \log(\frac{\pi_i}{1 - \pi_i})$. 
 \red{The model is established and fit in the code below.}
\begin{Code}
...
\end{Code}
 ...
 \red{Recall from Table \ref{tab:new_arguments_in_functions} that, by default, \fct{predict} returns predictions of the mean process, $\vec{\mu}$, and, if applicable, the probability process, $\vec{\pi}$: }
 We focus on $\vec{\pi}$, which can be interpreted as the proportion of families living below the poverty line. 
...
% and let $\vec{Q}_k$ denote the precision matrix of the basis-function coefficients at resolution $k$. 
 ...
 When the basis functions are regularly spaced (\code{regular = TRUE}), \pkg{FRK} v2 uses a precision matrix that is based on the Leroux model \citep{Leroux_2000_two_parameter_autoregressive_model}. 
 Let $\mathcal{N}_{i,k}$ denote the set of first-order horizontal and vertical neighbouring basis functions of the $i$th basis function of resolution $k$, 
%The number of elements in $\mathcal{N}_{ik}$, $|\mathcal{N}_{i,k}|$, will be four for interior basis functions, three for basis functions on edges, and two for basis functions on corners when the basis functions are arranged on a regular grid in $\mathbb{R}^2$.
 and let $\vec{Q}_k$ denote the precision matrix of the basis-function coefficients at resolution $k$. 
