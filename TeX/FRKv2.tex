...
\pkg{FRK} v1/v2 discretises the spatial domain of interest $D$ into $N$ small, non-overlapping basic areal units (BAUs) $\{A_i: i = 1, \dots, N\}$ such that $D = \cup_{i = 1}^N A_i$. 
...
\end{equation}
 where $\vec{T}$ and $\vec{S}$ are known design matrices constructed from $\vec{t}(\cdot)$ and $\vec{\phi}(\cdot)$ respectively, $\vec{\alpha}$ is a fixed effect and $\vec{\eta}$ is a random effect, and $\vec{\xi}$ is a vector associated with the fine-scale process which, like $\vec{\eta}$, is treated as a random effect.
...
%  Attributing point-referenced data to BAUs is straightforward.
  We then define the set of observation supports in terms of BAUs as $D^O \equiv \{B_j : j = 1, \dots, m\} $, where $B_j \equiv \cup_{i\in c_j} A_i$ is the representation of $R_j$ in terms of BAUs. 
 This approach to mapping data supports to BAUs is slightly different from \pkg{FRK} v1, which assumed that a spatial support contains a BAU if and only if the BAU centroid lies within the spatial support. This modification to \pkg{FRK} v2 caters for non-convex BAUs, such as those used in Section \ref{sec:spatialCOS}, where the centroid of a given BAU may lie outside of the BAU boundary. 
 Figure \ref{fig:BAU_intuition} shows a pedagogical example \red{with $m = 3$ observations} illustrating the relationship between the continuous domain $D$, the BAUs that make up $D$,  
the original spatial supports $\{R_1, R_2, R_3\}$, and the observation supports $\{B1, B_2, B_3\}$ in \pkg{FRK} v2.
...
In \pkg{FRK} v2, the weights $w_{ij}$ \red{may be controlled through the argument \mbox{\code{normalise\_wts}} and the \code{wts} field of the \class{Spatial*DataFrame}/\class{STFDF} object used to store the BAUs.} 
 ...
 By default (and implicit in \pkg{FRK} v1), each $v_i$ is set to 1. 
  The argument \mbox{\code{normalise\_wts}} controls whether $\vec{C}_Z$ corresponds to a weighted sum or a weighted average. If set to \code{FALSE}, then $w_{ij} = v_i$ for all $j$ (weighted sum); if set to \mbox{\code{TRUE}} (default and implicit in \pkg{FRK} v1), then the $\{w_{ij}\}$ are normalised so that each row of $\vec{C}_Z$ sums to 1 (weighted average) and $w_{ij} = v_i / \sum_{k \in c_j} v_k$.   
...
\end{gather}
 ...
 Note that \pkg{FRK} v2 is backwards compatible, since an identity link function and a Gaussian data model in (\ref{eqn:new_model_Z}), where $\psi$ plays the role of the measurement-error variance, \red{$\var{Z_j \mid Y_j}$ that does not depend on $j$}, yields the model used in \pkg{FRK} v1.
...
There are three principal quantities that could be of interest to the user, namely the latent process $Y(\cdot)$, the  mean process $\mu(\cdot)$, and data at unobserved locations. 
...
Recall that the Laplace approximation implies that, approximately, $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$; since $\vec{Y}$ is a linear function of $\vec{u}$, approximate inference on $Y(\cdot)$ can be done using well-known formulas. 
 ...
 We thus define $\vec{Y}_{\!\text{MC}}$, an $N \times n_{\text{MC}}$ matrix whose columns are the $n_{\text{MC}}$ MC samples of $\vec{Y} \mid \vec{Z}, \vec{\theta}$, as 
...
where every column of the matrix $\vec{A}$ is the estimated posterior mode of $\vec{\alpha}$, and each of the $n_{\text{MC}}$ columns of the matrix $\vec{U}$ are draws from  $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$.  
 ...
 Note that \pkg{FRK} v2 provides the user with $\vec{M}$ which, if needed, can be used to predict data with link function \red{$g(\cdot)$} but from a distribution that is different from that of the original data.
...
\subsubsection{Arbitrary prediction regions}
 ...
 Often, one does not wish to predict over single BAUs, but over regions spanning multiple BAUs, $\{\tilde{R}_k$, $k = 1, \dots, N_P\}$, where $N_P$ is the number of prediction regions. 
 ...
 That is, we write the indices of the BAUs associated with $\tilde{R}_k$ as $\tilde{c}_k \equiv \{i : A_i \cap \tilde{R}_k \neq \emptyset\}$, for $k = 1, \dots, N_P$.
 We then define the set of prediction regions \red{in terms of BAUs} as $D^P \equiv \{\tilde{B}_k : k = 1, \dots, N_P\}$, where $\tilde{B}_k \equiv \cup_{i\in \tilde{c}_k} A_i$ is the representation of $\tilde{R}_k$ in terms of BAUs. %, and where $\tilde{c}_k$ is a non-empty subset of $\{1, \dots, N\}$ that gives the indices of the BAUs associated with the $k$th user-specified region, $\tilde{R}_k$.
...
Prediction of $\vec{\mu}_P \equiv (\mu(\tilde{B}_1), \dots, \mu(\tilde{B}_P))^\tp$ over $D^P$ requires some form of aggregation across the associated BAUs. 
 ...
 Just as $\vec{\mu}_Z$ was constructed from the BAU-level mean process $\vec{\mu}$ via the matrix $\vec{C}_Z$ given by (\ref{eqn:C_Z}), since each $\tilde{B}_k$ is a BAU or a union of BAUs, one can construct an $N_P \times N$ matrix 
\[
\vec{C}_P \equiv \left(\tilde{w}_{ik}\mathbb{I}(i \in \tilde{c}_k) : i = 1, \dots, N; k = 1, \dots, N_P\right),
\]
such that
...
As in Section \ref{subsection:04-02:DataLayer}, the relative weights, $\{\tilde{v}_i: i = 1, \dots, N\}$, such that $\tilde{w}_{ik} \propto \tilde{v}_i$, are controlled by the \code{wts} field of the BAU object, and the argument \mbox{\code{normalise\_wts}} is used to control whether $\vec{C}_P$ represents a weighted sum or a weighted average. 
 For consistency between the model fitting and prediction stages, \pkg{FRK} v2 enforces the use of the same relative weights, $\tilde{v}_i = v_i$ for $i = 1, \dots, N$, and the same setting of \code{normalise\_wts}, in construction of both $\vec{C}_Z$ and $\vec{C}_P$. 
 ...
 Predictions and uncertainty quantification of the predictions can then be computed straightforwardly from $\vec{M}_P$. 
   ...
   We assume that these \red{data} are from the same exponential family model as that of the original data, $\vec{Z}$. %: That is, $Z^*_{Pk} \sim \text{EF}(\mu(\tilde{B}_k), \psi)$, $k = 1, \dots, N_P$.  
 MC samples of $\vec{Z}^*_P \equiv (Z^*_{P, 1}, \dots, Z^*_{P, N_P})^\tp$ can then be constructed straightforwardly using $\vec{M}_P$. 
 Note that \pkg{FRK} v2 provides the user with $\vec{M}_P$ which, if needed, can be used to predict data with link function \red{$g(\cdot)$} but from a distribution that is different from that of the original data. 
...
% For these distributions, we consider a probability process, $\pi(\vec{s})$, $\vec{s} \in D$, and the corresponding probability process evaluated over the BAUs, $\vec{\pi} \equiv (\pi_i : i = 1, \dots, N)^\tp$. 
% A user-specified function links the latent spatial process, $Y(\cdot)$, to the probability process, $\pi(\cdot)$, and then a distribution-specific function links the probability process to the mean process, $\mu(\cdot)$; see Appendix \ref{Appendix:Distributions with size parameters} for details. 

 \red{Software that cater for these distributions typically allow `link' functions such as the logit, probit, and complementary log-log functions.  
...
Instead, we use these functions to link the latent spatial process, $Y(\cdot)$, to a probability process, $\pi(\cdot)$: 
...
When modelling negative-binomial data, other popular link functions include the log and square-root functions. 
 In these cases, the BAU-level mean process is \mbox{$\vec{\mu} = \vec{k} \odot g^{-1}(\vec{Y})$}, where $g^{-1}(\cdot)$ is the inverse link function and  $\odot$ denotes element-wise multiplication.
%In these cases, the BAU-level mean process is \mbox{$\mu_i = k_ig^{-1}(Y_i)$}, $i = 1, \dots, N$, where $g^{-1}(\cdot)$ is the inverse link function.
 Then, the BAU-level probability process, $\vec{\pi}$, is constructed using (\ref{eqn:h_mu_equals_pi}). 
 }

 \red{Regardless of the chosen link function, we define the probability process over the observation supports, $B_j$, $j = 1, \dots, m$, as $\vec{\pi}_Z \equiv (\pi_{Z_j} : j = 1, \dots, m)^\tp$, where
\begin{equation}
	\pi_{Z_j} = h(\mu_{Z_j}; k_{Z_j}), \quad j = 1, \dots, m.
\end{equation}
 Similarly, we define the probability process over arbitrary prediction regions 
%% Got into some problem with this; I've used k as the indices of the prediction regions. 
% $\tilde{B}_k$, $k = 1, \dots, N_P$, 
%  \begin{equation}
%	\pi(\tilde{B}_k) = h(\mu_{P_k}; k_{Z_j}), \quad j = 1, \dots, m.
%\end{equation}
 as \mbox{$\vec{\pi}_P \equiv h(\vec{\mu}; \vec{k}_P)$}, where $h(\cdot; \cdot)$ is applied elementwise and $\vec{k}_P \equiv \vec{C}_P \vec{k}$.  
}

  

 \red{When model fitting, if each observation is associated with exactly one BAU, the user can provide the data-level size parameters $\{k_{Z_j} : j = 1, \dots, m\}$ only, which are used to infer the BAU-level size parameters at observed BAUs; in particular, $k_i = \sum_{j \in a_i} k_{Z_j}$, $i = 1, \dots, N$, where $a_i$ denotes the indices of the observations associated with BAU $A_i$.}  
 In the case where one or more observations encompass multiple BAUs, size parameters are set at the BAU level: Data size parameters are then selected from the associated BAU-level size parameters, \mbox{$\{k_i : i = 1, \dots, N\}$}, and summed; that is, $k_{Z_j} = \sum_{i \in c_j} k_i$, $j = 1, \dots, m$. %or, equivalently, $\vec{k}_Z = \vec{C}_Z \vec{k}$. 
...
See the package documentation for details.
 ...
 Therefore, with these distributions, \pkg{FRK} v2 requires that the matrix $\vec{C}_Z$ in (\ref{eqn:C_Z}) is constructed with the relative weights \mbox{$\{v_i = 1$ : $i = 1, \dots, N\}$} and with \mbox{\code{normalise\_wts = FALSE}}, and hence \red{$w_{ij} = 1$} in (\ref{eqn:C_Z}). 
...
% In what follows, \red{$\text{Pois}(\lambda)$ refers to a probability distribution on the non-negative integers, namely $p_j =e^{-\lambda} \lambda^j/j!$ for $j = 0, 1, \dots$.}  
 In what follows, \red{$\text{Pois}(\mu)$ refers to a probability distribution on the non-negative integers, namely $P(Z = x) = e^{-\mu} \mu^x/x!$ for $x = 0, 1, \dots$.}  
...
Next, an \class{SRE} object is constructed using \fct{SRE}, within which we specify the data model, the link function, and the parameterisation of $\cov{\vec{\eta}}{\vec{\eta}}$.  
 In this example, we model counts using a Poisson data model, $Z_j \mid \vec{\mu} \sim \text{Pois}(\mu_{Z_j})$ \red{at given observation support $B_j$}, $j = 1, \dots, m$, and we use the log link function, $g(\cdot) = \log(\cdot)$. 
...
%\begin{Code}
%R> S <- FRK(f = Z ~ 1, data = Poisson_simulated,
...
\begin{figure}[t!]
    ...
    \vspace{-20pt}  
    \caption{Predictions and prediction-interval widths returned by \pkg{FRK} v2 when applied to the simulated Poisson data shown in the right panel of Figure \ref{fig:Poisson_true_and_Z}. (Left panel) Prediction of the latent process over the BAUs, $\vec{Y}$. (Centre-left panel) Width of the 90\% prediction interval for each element of $\vec{Y}$. (Centre-right panel) Prediction of the mean process over the BAUs, $\vec{\mu} = g^{-1}(\vec{Y})$. (Right panel) Width of the 90\% prediction interval for each element of $\vec{\mu}$.}
...
That is, some data supports are large areal regions comprising many BAUs, while others coincide with the BAUs.  
 ...
 Finally, we simulate negative-binomial data, \red{\mbox{$Z_j \mid \vec{\mu}, \vec{k}_Z \sim \text{NB}(k_{Z_j}, \pi_{Z_j})$}}, $j = 1, \dots, m$, shown in the right panel of Figure \ref{fig:03-02-negative-binomial_data}. 
% \red{Here, $\text{NB}(\lambda, k)$ is a probability distribution on the non-negative integers, namely $p_j = \Gamma(j + k)/(\Gamma(k)j!)\lambda^k(1-\lambda)^j$ for $j = 0, 1, \dots$.}
  \red{Here, $\text{NB}(k, \pi)$ is a probability distribution on the non-negative integers, namely \mbox{$P(Z = x) = {{x + k - 1}\choose x}\pi^k(1-\pi)^x$} for $x = 0, 1, \dots$.}
...
\begin{figure}[t!]
    ...
    In the centre-right and right panels, the white regions are unobserved. 
...
\end{figure}
 ...
 In this example, the data model is negative-binomial, that is, $Z_j \mid \vec{\mu}, \vec{k}_Z \sim \text{NB}(\mu_{Z_j}, k_{Z_j})$, $j = 1, \dots, m$, and we use the logit `link' function $g(\cdot) = \log\left(\frac{\cdot}{1 - \cdot}\right)$ (see Appendix \ref{Appendix:Distributions with size parameters}). 
 Since some observations are associated with multiple BAUs, we must provide the BAU-level size parameters (see Section \ref{sec:Distributions with size parameters}).  
\begin{Code}
R> BAUs$k_BAU <- 50 
...
To illustrate that user-specifed prediction regions, $\tilde{R}_k$, $k = 1, \dots, N_P$, are arbitrary, we demonstrate prediction over a handful of irregularly-shaped areas. % (defined as a \class{SpatialPolygons} object).

\begin{Code}
R> pred <- predict(S, newdata = arbitrary_polygons)
\end{Code}
Figure \ref{fig:03-02-negative-binomial_polygons} shows the predictions and prediction-interval widths, respectively, for the \red{mean process evaluated over the irregularly-shaped areas, $\vec{\mu}_P$.}  

\begin{figure}[t!]
    \centering
    \includegraphics[width = \linewidth]{img/Negbinom_sim_arbitrary_polygon_predictions.png}
	\vspace{-20pt}    
    \caption{Prediction (left panel) and prediction-interval widths (right) for the aggregated mean, $\vec{\mu}_P$, when predicting over arbitrary prediction regions, $\tilde{R}_k$, $k = 1, \dots, N_P$, in the negative-binomial example of Section \ref{sec:03-03:negative-binomial}. Note that these polygons have equal area.}
...
\begin{table}[t!]
    ...
    \caption{\red{Scores for each competing method in the comparative study presented in \cite{Heaton_2019_comparative_study}.}}
...
\end{table}
 ...
 In our second experiment, we re-ran the analysis for the comparative study published in \cite{Heaton_2019_comparative_study}, which included spatial predictions from \pkg{FRK} v1.
...
%The training set consists of 105,569 observations, while the test set consists of 42,740 observations.
 ...
 The results show that the increased number of basis functions from \pkg{FRK} v2 significantly improved the diagnostic scores over \pkg{FRK} v1, so that the results for \pkg{FRK} v2 are now comparable to the heretofore-preferred MRA.  
 To achieve these improvements over \pkg{FRK} v1, we only had to specify \code{nres = 4}, \code{K\_type = "precision"} and \code{method = "TMB"} in \fct{auto\_basis}, \fct{SRE}, and \fct{SRE.fit}, respectively; the rest of the \pkg{FRK} v1 code as used in the comparative study was left unchanged. 
...
In Section \ref{sec:ST_example}, we provide a non-Gaussian spatio-temporal example through modelling crime counts in the city of Chicago in the first two decades of the 21st century.
...
We considered two types of sampling schemes for model comparison.
 ...
 The chosen training block was a 30 $\times$ 30 square (900 pixels) in the middle of the spatial domain of interest, leaving 32,850 pixels for testing. 
The training and test sets under the two sampling schemes are shown in Figure \ref{fig:03-04-Modis1}. 
% \red{In what follows, $\text{Bin}(\lambda, k)$ refers to a probability distribution on the integers $\{0, 1, \dots, k\}$, namely $p_j = {k \choose j}\lambda^j(1-\lambda)^{k-j}$. For the special case of $k = 1$, $\text{Bin}(\lambda, 1)$ is the Bernoulli distribution. }
  \red{In what follows, $\text{Bin}(k, \pi)$ refers to a probability distribution on the integers $\{0, 1, \dots, k\}$, namely $P(Z = x) = {k \choose x}\pi^x(1-\pi)^{k-x}$. For the special case of $k = 1$, $\text{Bin}(1, \pi)$ is the Bernoulli distribution. }
...
\end{figure}
 ...
 In this case study, the data model is \red{$Z_j \mid \vec{\mu} \sim \text{Bin}(1, \pi_{Z_j})$, $j = 1, \dots, m$, where $\pi_{Z_j}$} represents the probability of `Cloud' over the pixel $R_j$. 
 ...
 For the methods requiring specification of a link function, we used the standard logit link function\red{, $f(\cdot) = \log(\frac{\cdot}{1 - \cdot})$}.
...
\begin{figure}
    ...
    \vspace{-25pt}
     \caption{Predictions of the probability of `Cloud' on the test set of the missing-at-random experiment shown in Figure \ref{fig:03-04-Modis1}.  The corresponding test data is shown in the top-left panel. Note that the training locations are indicated by white pixels.}   
...
\begin{figure}[t!]
    ...
    \vspace{-25pt}
     \caption{Predictions of the probability of `Cloud' on the test set of the `missing-in-a-block' experiment shown in Figure \ref{fig:03-04-Modis1}. Here, we only show the test locations, which correspond to the 30 $\times$ 30 block near the centre of the spatial domain; the test data in this block is shown in the left-most panel of this figure.}   
...
The predictions from \pkg{FRK}, \pkg{INLA}, and \pkg{spNNGP} are similar, 
 ...
 The packages \pkg{FRK} and \pkg{INLA} return predictive probabilities close to 0.5, while \pkg{mgcv} and \pkg{spBayes} are more confident in their predictions. 
...
\end{table}
 ...
 To assess predictive accuracy, we compared the predictions from all models using the Brier score \citep[Sec.~3]{Gneiting_2007_scoring_rules}, and the area under the receiver operating characteristic (ROC) curve (AUC).
 ...
 For the MR sampling scheme, there is little discernible difference between \pkg{FRK}, \pkg{INLA}, \pkg{mgcv}, and \pkg{spNNGP}. 
 ...
 The task of prediction over a completely unobserved region is challenging, and so it is no surprise that the diagnostics for the MB sampling scheme are worse than for the MR sample scheme. 
 ...
 Note that all run times increased under the MB scheme; however, \pkg{FRK} increased by a factor of less than 2, while increases in the run times for other packages were between a factor of 3 and 10. 
 Given that the training sample size is significantly larger in the MB scheme than under the MR scheme (32,850 pixels compared to 6,000 pixels), this suggests that \pkg{FRK} is well suited to fitting and predicting with large sample sizes.  
 Overall, these results suggest that \pkg{FRK} is comparable to, or better than, other packages in this application. 
 The main advantages of \pkg{FRK} however lie in the ease with which it allows one to do other more elaborate analyses with spatial or spatio-temporal non-Gaussian data of differing support, as shown in the next sections. 
...
% Note that \pkg{FRK} v2 does not explicitly cater for a lognormal response, but an effective approach is to use \code{response = "Gaussian"} and \code{link = "log"}.  
 ...
 These blocking schemes represent the user-specified prediction regions, $\tilde{R}_k$, $k = 1, \dots, 10$. 
...
As in \cite{Paul_Cressie_2011_lognormal_kriging_block_prediction}, we use a piecewise linear trend in the `distance from GZ' as a fixed effect: Specifically, the log of the observations within a distance of 30.48m (100 ft) from GZ are assumed to follow a different trend to those observations beyond 30.48m from GZ, making up two regimes depending on distance from GZ. 
 The following code constructs the BAU-level covariates that are needed to fit this piecewise linear trend: \code{BAUs\$x1} and \code{BAUs\$x3} are indicator variables used to model the intercepts in each regime, and \code{BAUs\$x2} and \code{BAUs\$x4} are used to model the slopes of the trend in each regime. 
...
Spatial statistical modelling for this problem is done by setting \code{response = "Gaussian"} and \code{link = "log"} when calling \fct{FRK}. 
 ...
 Note that in the formula below, we suppress the global intercept since we model the region-specific intercepts separately through \code{BAUs$x1} and \code{BAUs$x3}. 
\begin{Code}
R> Am_data$std <- 1 
...
To validate our predictions, we used the \proglang{R} package \pkg{georob} version 0.3.14 \citep{georob}, which implements an unbiased back-transformation of kriging predictions of log-transformed data \citep{Cressie_2006_block_kriging_lognormal_spatial_processes}.  
...
\includegraphics[width = \linewidth]{img/Sydney_training_data_mixed.png}
    ...
    (Right panel) The observed proportion of families in poverty computed by dividing the number of families in poverty by the total number of families, that is, $Z_j / k_{Z_j}$, $j = 1, \dots, m$. % at the SA2 level, 
...
It is often the case that sampling once from a large area is relatively inexpensive compared to acquiring multiple samples from small areas. 
 ...
 \red{Our data model is $Z_j \mid \vec{\mu}, \vec{k}_Z \sim \text{Bin}(k_{Z_j}, \pi_{Z_j})$, $j = 1, \dots, m$,} and we use the logit link function, $f(\cdot) = \log(\frac{\cdot}{1 - \cdot})$.
\begin{Code}
R> S <- FRK(f = total_poverty_count ~ 1, 
+    data = SA2_and_some_SA1s, BAUs = SA1s, 
...
\end{Code}
 Since the SA1 regions are our BAUs, we can predict both the probability process, $\vec{\pi}$, and the mean process, $\vec{\mu}$, over the SA1 regions: We focus on $\vec{\pi}$, which can be interpreted as the proportion of families living below the poverty line. 
 ...
 \begin{figure}[t!]
    ...
    \includegraphics[width = \linewidth]{img/Sydney_SA1_predictions_mixed.png}
        \vspace{-15pt}
    \caption{SA1-level predictions. (Left panel) Prediction of the probability process over the SA1 regions, $\vec{\pi}$, representing the proportion of families in each region. (Right panel) 90\% prediction-interval width for \red{each element of} $\vec{\pi}$.
...
% To predict over the SA3 regions, we simply set \code{newdata} to a  \class{SpatialPolygonsDataFrame} object containing the SA3 regions.
 \green{We predict over the SA3 regions by passing them as a \class{SpatialPolygonsDataFrame} object to the argument \code{newdata}. }
\begin{Code}
R> SA3_predictions <- predict(S, newdata = SA3s)
\end{Code}
Figure \ref{fig:SA3_predictions} shows the SA3-region predictions and associated prediction-interval widths for the mean process, $\vec{\mu}_P$. Again, this graphic was generated using \fct{plot}. 

\begin{figure}[t!]
    \centering
    \includegraphics[width = \linewidth]{img/Sydney_SA3_predictions_mixed.png}
        \vspace{-15pt}
    \caption{SA3-level predictions. (Left panel) Prediction of the mean process over the SA3 regions, $\vec{\mu}_P$, representing the expected number of families in poverty in each region. (Right panel) The 90\% prediction-interval width for \red{each element of} $\vec{\mu}_P$. 
    For details on the underlying Stamen base map, refer to the caption of Figure \ref{fig:SA2_level_data}.
}   
...
Then, we initialise and fit the \class{SRE} object using \fct{FRK}, setting \code{response = "poisson"} and \code{link = "log"} in order to use a Poisson data model, $Z_j \mid \vec{\mu} \sim \text{Pois}(\mu_{Z_j})$, $j = 1, \dots, m$, and a log link function, $g(\cdot) = \log(\cdot)$. 
...
\section{Conclusion}\label{SEC:Conclusion}
 ...
 In this paper, we have described  \pkg{FRK} v2, which is a major upgrade to \pkg{FRK} v1. Substantial enhancements allow for the spatial and spatio-temporal modelling of, and large-scale prediction from, big, non-Gaussian data. 
...
and Yi Cao for technical assistance during several stages of the project. 
...
\subsection[Covariance matrix K]{Covariance matrix $\vec{K}$}\label{Appendix:CovarianceTapering:covariance_matrix_K}
 ...
 In \pkg{FRK} \red{v1}/v2, we let $K_k(\vec{s}, \vec{s}^*)$ be the exponential covariance function, that is, 
...
% and let $\vec{Q}_k$ denote the precision matrix of the basis-function coefficients at resolution $k$. 
 ...
 When the basis functions are regularly spaced (\code{regular = TRUE}), \pkg{FRK} v2 uses a precision matrix that is \red{based on the Leroux model \citep{Leroux_2000_two_parameter_autoregressive_model}}. 
 Let $\mathcal{N}_{i,k}$ denote the set of first-order horizontal and vertical neighbouring basis functions of the $i$th basis function of resolution $k$, 
%The number of elements in $\mathcal{N}_{ik}$, $|\mathcal{N}_{i,k}|$, will be four for interior basis functions, three for basis functions on edges, and two for basis functions on corners when the basis functions are arranged on a regular grid in $\mathbb{R}^2$.
 and let $\vec{Q}_k$ denote the precision matrix of the basis-function coefficients at resolution $k$. 
...
Note that \pkg{LatticeKrig} uses $\vec{Q}_k^\tp\vec{Q}_k$ as the precision matrix blocks, whereas \pkg{FRK} v2 uses $\vec{Q}_k$.  
 ...
 To cater for irregularly-spaced basis functions, \pkg{FRK} v2 also offers a sparse precision matrix considers the distance between basis-function centroids:  
...
% is the vector of BAU-level size parameters. 
 ...
 \red{Recall from Section \ref{sec:Distributions with size parameters} that $h(\cdot\,; \cdot)$ is a function used in (\ref{eqn:h_mu_equals_pi}) to link the BAU-level probability process, $\vec{\pi}$, to the BAU-level mean process, $\vec{\mu}$. 
 In this appendix, we give its derivation.  
  The expectation of a binomial random variable, $Z \mid \pi, k \sim \text{Bin}(k, \pi)$, 
%  The expectation of a binomial random variable \red{$Z$, with $k$ trials and with probability of success parameter $\pi$},  
  is $\E{Z} = k\pi$, 
%  which implies that $\pi = \E{Z}/k$ and 
  which motivates the use of
...
%  The expectation of a negative-binomial random variable $Z$, with a target number of successes $k$ and with probability of success parameter $\pi$,     
 is $\E{Z} = \frac{k(1 - \pi)}{\pi}$, 
% which implies that $\pi = \frac{k}{\E{Z} + k}$ and 
 which motivates the use of
  \begin{equation}\label{eqn:h_neg_binomial}
 h(\mu; k) = \frac{k}{\mu + k},
 \end{equation}
 when the response distribution is negative-binomial. 
 By construction, using $h(\cdot; \cdot)$ to link the probability process to the mean process as described in Section \ref{sec:Distributions with size parameters}, ensures that the range of the mean process is appropriate for modelling the expectation of a binomial distributed random variable where the mean must lie in the range $(0, k)$, or a negative-binomial distributed random variable where the mean must lie in the range $(0, \infty)$.}
...
% Two distributions considered in this framework, namely, the binomial distribution and the negative-binomial distribution, have an assumed-known `size' parameter and a `probability of success'
%parameter. 
...
% These functions are clearly inappropriate for direct use in (\ref{eqn:mu_linked_to_Y}), as their inverse functions have a range of $(0, 1)$, which would unacceptably restrict the range of the mean process. 
%Instead, we use these functions to define the probability process, 
% \begin{equation}\label{eqn:prob_process}
%    f(\pi(\vec{s})) = Y(\vec{s}), \quad \vec{s} \in D,
%\end{equation}
%where $f(\cdot)$ is one of the aforementioned functions whose inverse has a range of $(0, 1)$. 
...
%
% 
%The function $h(\cdot\,; \cdot)$ is derived from the formula for the expectation of the response distribution. 
...
% \end{equation}
% when the response distribution is negative-binomial. 
% By construction, this approach ensures that the range of the mean process is appropriate for modelling the expectation of a binomial distributed random variable where the mean must lie in the range $(0, k)$, or a negative-binomial distributed random variable where the mean must lie in the range $(0, \infty)$.
% 
%When modelling negative-binomial data, other popular link functions include the log and square-root functions. 
%%The inverse of these functions have a range of $(0, \infty)$, which is appropriate for modelling the expectation of a negative-binomial distributed random variable, and hence can be used directly in (\ref{eqn:mu_linked_to_Y}). 
%%%Of course, the size parameters still need to be accounted for, which is done by simple multiplication.
%%However, the mean of a negative-binomial distribution cannot be defined without specification of a size parameter: 
%In these cases, the mean process evaluated over the BAUs is \mbox{$\vec{\mu} = \vec{k} \odot g^{-1}(\vec{Y})$}, where $g^{-1}(\cdot)$ is the inverse link function and  $\odot$ denotes element-wise multiplication. 
...
\begin{itemize}
    ...
    \end{equation*}
    \item (Empirical) mean-absolute error (MAE): The (empirical) MAE, also used to assess point-wise predictive performance, is
    \begin{equation*}
        \textrm{MAE}
        \equiv
        \frac{1}{|D^*|}\sum_{\vec{s} \in D^*}|\hat{\mu}(\vec{s}) - \mu(\vec{s})|.
    \end{equation*}
    \item (Empirical) mean-absolute percentage error (MAPE): The (empirical) MAPE, is similar to the MAE, but considers relative error instead:
    \begin{equation*}
        \textrm{MAPE}
        \equiv
...
%    \textrm{CRPS}(F, \mu(\vec{s})) 
    ...
    \item (Averaged) interval score \citep[IS;][sec.~6.2]{Gneiting_2007_scoring_rules}: 
%    The interval score of predictions of $\mu(\vec{s})$, $\vec{s} \in D^*$, for a purported $(1-\alpha)\times 100$\% prediction interval, is defined as
    Given a set of purported 
    $(1-\alpha)\times 100$\% prediction intervals for $\mu(\vec{s})$, $\vec{s} \in D^*$, the (averaged) IS is defined as
    \begin{align*}
    \text{IS}_\alpha
    \equiv 
    ...
    \end{equation*}
    If the intervals are indeed $(1-\alpha)\times 100$\% prediction intervals, the (empirical) coverage should be approximately equal to $1-\alpha$.
...
Unfortunately, the family units in the 2011 Australian Census do not align exactly with those used by the MIAESR, and so, since this example is shown for purely illustrative purposes, we make several assumptions with the Census data. 
