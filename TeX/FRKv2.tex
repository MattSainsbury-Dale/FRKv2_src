...
\textit{data layer}, we use a conditionally independent exponential-family model for each element of the data vector; see Section \ref{subsection:04-02:DataLayer}. 
 \green{In Section \ref{sec:Distributions with size parameters}, we consider distributions that have an assumed-known `size' parameter, %associated with each datum, 
 namely the binomial distribution and the negative-binomial distribution.} 
...
As in \pkg{FRK} v1, the elements of $\vec{\xi}$ are modelled as independent and identically distributed (i.i.d.) Gaussian random variables with variance $\sigma^2_\xi$, and $\vec{\eta}$ is modelled as a mean-zero multivariate-Gaussian random variable with covariance matrix $\cov{\vec{\eta}}{\vec{\eta}}$. 
 ...
 Both formulations use block-diagonal matrices, so that basis-function coefficients between basis-function resolutions are independent; see Appendix \ref{Appendix:CovarianceTapering} for how the intra-resolution dependencies with $\vec{K}$ and $\vec{Q}$ are modelled. 
  In a non-Gaussian setting (when \pkg{TMB} is used for model fitting), use of $\vec{Q}$ instead of $\vec{K}$ is computationally advantageous.\footnote{\red{Andrew, can we discuss your suggested edit. The matrix $\vec{Q}$ is preferred because \pkg{TMB} is significantly more efficient when using a sparse precision matrix: Even when the blocks of $\vec{K}$ are sparse (e.g., through covariance tapering), its inverse is dense, which makes \pkg{TMB} slow down.}} 
...
Define the mean of the observations as $\vec{\mu}_Z \equiv (\E{Z_1}, \dots, \E{Z_m})^\tp$. 
...
%  Note that if $v_i = |A_i|$ and \mbox{\code{normalise\_wts = TRUE}}, the $j$th row sum of $\vec{C}_Z$ is $\sum_{i \in c_j} |A_i| = |B_j|$, so that the normalised weights are $w_{ij} = |A_i|/|B_j|$.  
  Note that if $v_i = |A_i|$ and \mbox{\code{normalise\_wts = TRUE}}, the normalised weights are $w_{ij} = |A_i|/|B_j|$, since \green{the BAUs are disjoint and} $\sum_{i \in c_j} |A_i| = |B_j|$. 
...
\subsection{Distributions with size parameters}\label{sec:Distributions with size parameters}
 ...
 \green{Two distributions considered in this framework, namely the binomial distribution and the negative-binomial distribution, have an assumed-known `size' parameter and a `probability of success'
parameter. 
Given %data, $\vec{Z} \equiv (Z_1, \dots, Z_m)$, and 
the vector of size parameters associated with the data, $\vec{k}_Z \equiv (k_{Z1}, \dots, k_{Zm})^\tp$, the parametrisation used in \pkg{FRK} v2 assumes that: For binomial data models, $Z_j$ represents the number of `successes' from $k_{Zj}$ trials; for negative-binomial data models, $Z_j$ represents the number of failures before $k_{Zj}$ successes.}

\green{
 For these distributions, we consider a probability process, $\pi(\vec{s})$, $\vec{s} \in D$, and the probability process evaluated over the BAUs, $\vec{\pi} \equiv (\pi_i : i = 1, \dots, N)^\tp$. 
 We use a user-specified link function to link the latent spatial process, $Y(\cdot)$, to the probability process, $\pi(\cdot)$, and then a distribution-specific link function to link the probability process to the mean process, $\mu(\cdot)$; see Appendix \ref{Appendix:Distributions with size parameters} for details. 
  }

 \green{
% As discussed in Appendix \ref{Appendix:Distributions with size parameters}, the construction of $\vec{\mu}$ requires the specification of size parameters at the BAU level, $\vec{k} \equiv (k_i: i = 1, \dots, N)^\tp$. 
 In general, \pkg{FRK} v2 needs the size parameters for every observed BAU. 
 ...
 When some observations are associated with multiple BAUs, the user must provide the size parameters at the BAU level:  % (at least for all observed BAUs).
 In this case, if $\vec{k}_Z$ is not provided explicitly, it is obtained by summing the BAU-level size parameters. %Specifically, $$\vec{k_Z} = \vec{C}_Z \vec{k}.$$
 }

\green{In most applications considering binomial or negative-binomial data models, 
%  that is, when $Z_j \sim \text{Bin}(\mu_{Zj}; k_{Zj})$ or $Z_j \sim \text{NB}(\mu_{Zj}; k_{Zj})$, $j = 1, \dots, m$, 
 the mean of an observation is treated as a simple aggregate of the underlying mean process. Therefore, with these distributions, the matrix $\vec{C}_Z$ in (\ref{eqn:C_Z}) is constructed with the relative weights \mbox{$\{v_i = 1$ : $i = 1, \dots, N\}$} and with \mbox{\code{normalise\_wts = FALSE}}. Then, the mapping from the BAU-level mean, $\vec{\mu}$, to the data-level mean, $\vec{\mu}_Z$, in (\ref{eqn:04-01:mu_Z}) is a simple, un-weighted summation over the BAUs.}
 


% \green{When the response distribution is binomial or negative-binomial, (\ref{eqn:new_model_Z}) 
...
where every column of the matrix $\vec{A}$ is the estimated posterior mode of $\vec{\alpha}$, and each of the $n_{\text{MC}}$ columns of the matrix $\vec{U}$ are draws from  $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$.  
 We obtain MC samples of the $N$-dimensional vector $\vec{\mu}$ from $[\vec{\mu} \mid \vec{Z}, \vec{\theta}]$ via $\vec{M} \equiv g^{-1}(\vec{Y}_{\!\text{MC}})$, where $g^{-1}(\cdot)$ is applied element-wise. 
  \green{\pkg{FRK} v2 also allows inference on data over all $N$ BAUs, $\vec{Z}^* \equiv (Z^*_1, \dots, Z^*_N)$. We assume that these data follow the same exponential family model as that used for the original data, $\vec{Z}$. %: That is, $Z^*_i \sim \text{EF}(\mu_i, \psi)$, $i = 1, \dots, N$.  
 MC samples of $\vec{Z}^*$ can then be constructed straightforwardly using $\vec{M}$. %, since the distribution of an exponential family member depends only on its conditional mean (and possibly a dispersion parameter, which we model as being constant throughout the spatial domain). 
 Note that \pkg{FRK} v2 provides the user with $\vec{M}$, which can facilitate simulation from a different distribution to that used for the original data.}
...
As in Section \ref{subsection:04-02:DataLayer}, \green{the relative weights, $\{v_i: i = 1, \dots, N\}$, such that $\tilde{w}_{ik} \propto v_i$, are controlled by the \code{wts} field of the BAU object, and the argument \mbox{\code{normalise\_wts}} is used to control whether $\vec{C}_P$ represents a weighted sum or a weighted average. 
 For consistency between the model fitting and prediction stages, \pkg{FRK} v2 requires that the same relative weights, $\{v_i: i = 1, \dots, N\}$, and the same setting of \code{normalise\_wts} are used in construction of both $\vec{C}_Z$ and $\vec{C}_P$.}
 ...
 MC samples of $\vec{\mu}_P \mid \vec{Z}, \vec{\theta}$ can be constructed  via $\vec{M}_P \equiv \vec{C}_P \vec{M}$, where recall that $\vec{M}$ consists of MC samples from $[\vec{\mu} \mid \vec{Z}, \vec{\theta}]$. Predictions and uncertainty quantification of the predictions can then be computed straightforwardly from $\vec{M}_P$. 
   \green{\pkg{FRK} v2 also allows inference on data over aggregations of BAUs, $\vec{Z}^*_P \equiv (Z^*_{P1}, \dots, Z^*_{PN_P})$. 
   We assume that these data follow the same exponential family model as that used for the original data, $\vec{Z}$. %: That is, $Z^*_{Pk} \sim \text{EF}(\mu(\tilde{B}_k), \psi)$, $k = 1, \dots, N_P$.  
 MC samples of $\vec{Z}^*_P$ can then be constructed straightforwardly using $\vec{M}_P$. %, since the distribution of an exponential family member depends only on its conditional mean (and possibly a dispersion parameter, which we model as being constant throughout the spatial domain). 
 Note that \pkg{FRK} v2 provides the user with $\vec{M}_P$, which can facilitate simulation from a different distribution to that used for the original data.}
...
We then construct the mean process evaluated over the BAUs, $\vec{\mu}$, which is shown in the centre-left panel of Figure \ref{fig:03-02-negative-binomial_data}; as we are simulating negative-binomial data, this requires specification of assumed-known `size' parameters and, for simplicity, we set $k_i = 50$, $i = 1, \dots, N$. 
% Next, we define the data supports, $R_j$, $j = 1, \dots, m$, which are constructed in terms of the BAUs, so that $R_j$ and $B_j$ are equivalent in this example.  
  Next, we define a set of data supports consisting of both large-scale and fine-scale observations: 
% To get a handle on the fine-scale-variation parameter, we include both large-scale and fine-scale observations: 
That is, some data supports are large areal regions comprising many BAUs, while others coincide with the BAUs.  
 ...
 Finally, we simulate negative-binomial data, $Z_j \sim \text{NB}(\mu_{Zj}, k_{Zj})$, $j = 1, \dots, m$,  shown in the right panel of Figure \ref{fig:03-02-negative-binomial_data}.}
...
\begin{figure}[t!]
    ...
    \green{In the centre-right and right panels, the white regions are unobserved.} 
%    In this example, we have both large-scale and fine-scale observations: That is, some data supports are large areal regions comprising many BAUs, while other data supports coincide with the BAUs. 
}   
...
\green{In this example, the data model is negative-binomial, that is, $Z_j \sim \text{NB}(\mu_{Zj}, k_{Zj})$, $j = 1, \dots, m$, and we use the logit `link' function (see Appendix \ref{Appendix:Distributions with size parameters}).} 
...
Figure \ref{fig:03-02-negative-binomial} shows the predictions and prediction-interval widths for the $\vec{\pi}$ and $\vec{\mu}$. 
...
\red{In this example, our predictions and figures are all about the process. We of course do not have the true values of the process, so it is not possible to get prediction intervals directly. Instead, the below coverage results are found by simulating from the \textit{data model}, and then comparing to our observed. Does this need to be made explicit?}
...
\red{In this example I predict \textit{data} over the BAUs. Should I add a sentence or two explicitly saying this, and that the assumption of the same EF model as that used for the original data is reasonable given i) the model is Poison and ii) the BAUs and the original data supports coincide?}
...
\begin{itemize}
	\item \red{Should we mention that we do not cater for an offset term in the Poisson case?} 
	\item \red{I believe our model falls into the `global smoothing' category: I read that global models include the potential for oversmoothing, as
discontinuities between adjacent areas are smoothed over. I suppose this is alleviated by the inclusion of the independent random effects $\vec{\xi}$.}
\end{itemize}
...
and \green{Yi Cao for technical assistance during several stages of the project}. 
...
We note that $\vec{Q}_k$ is diagonally dominant, and hence it is positive-definite. 
 ...
 \red{LeRoux model: Leroux BG, Lei X, Breslow N, 2000. Estimation of disease rates in small areas: a new mixed model
for spatial dependence. In: M. E. Halloran and D. Berry (eds.), Statistical models in
epidemiology, the environment and clinical trials. Springer, New York.}
...
\section{Link functions for distributions with size parameters}\label{Appendix:Distributions with size parameters}


 \green{Two distributions considered in this framework, namely, the binomial distribution and the negative-binomial distribution, have an assumed-known `size' parameter and a `probability of success'
parameter. 
%Given data, $\vec{Z} \equiv (Z_1, \dots, Z_m)$, and assumed-known `size' parameters, $\vec{k}_Z \equiv (k_{Z1}, \dots, k_{Zm})^\tp$, the parametrisation used in \pkg{FRK} v2 assumes that: For binomial data models, $Z_j$ represents the number of `successes' from $k_{Zj}$ trials; for negative-binomial data models, $Z_j$ represents the number of failures before $k_{Zj}$ successes. 
...
where $f(\cdot)$ is one of the aforementioned functions whose inverse has a range of $(0, 1)$. 
 ...
 Next, we link the mean process at the BAU level to the probability process at the BAU level, 
  \begin{equation}
 h(\mu_i; k_i) = \pi_i, \quad i = 1, \dots, N, 
 \end{equation}
 where $h(\cdot; \cdot)$ is a function determined solely by the response distribution, and $\vec{k} \equiv (k_1, \dots, k_N)^\tp$ is the vector of assumed-known `size' parameters at the BAU level. 
...
\green{The function $h(\cdot; \cdot)$ is derived from the formula for the expectation of the response distribution. 
...
\green{When modelling negative-binomial data, a popular choice of link functions are the log and square-root functions. The inverse of these functions has a range of $(0, \infty)$, which is appropriate for modelling the expectation of a negative-binomial distributed random variable, and hence can be used directly in (\ref{eqn:mu_linked_to_Y}). Of course, the size parameters still need to be accounted for, which is done by simple multiplication.}
