...
%TODO Do this on campus, it will be much faster.

%TODO  .tar.gz file that weren't updated but I'm happy for you to submit and these could be changed later, maybe even after acceptance... namely the figures need to be made the same as in the paper (currently they're different) and files could be cleaned up (e.g., #OUTPUT#). 
%TODO  could the `._*` files be purged from the file `FRKv2_renv_docker.tar.gz`?.
...
%TODO All scripts should be in the subdirectory Code/, and all figures produced by the scripts, if they are written to file, should be written to Figures/.
...
The \class{DHARMa} object can then be used to construct residual plots and to perform a range of hypothesis tests, including tests for residual spatial dependence and zero-inflation, as well as arbitrary, user-defined tests. 
 ...
 %\pkg{FRK}~v2 also provides a function, \fct{diagnostics}, which, given a sample from \fct{posteriorpredictive} and the corresponding observed data, computes several default (e.g., coverage) and user-defined diagnostics.  
  ...
  \begin{figure}[t!]
    \centering
    \includegraphics[width = \linewidth]{results/3_2_Poisson_sim_residuals.pdf}
    \vspace{-20pt}  
