...
\end{equation}
 where $\vec{Y} \equiv (Y_1 \dots, Y_N)^\tp$ is an $N$-dimensional vector corresponding to the BAUs $\{A_i, \dots, A_N\}$, $\vec{T}$ and $\vec{S}$ are known design matrices, and $\vec{\xi}$ is the vector associated with the fine-scale process. 
...
%
%
%When the basis functions are regularly spaced, \pkg{FRK} v2 also offers a \pkg{LatticeKrig} \citep{Nychka_2016_LatticeKrig} style prior wherein a sparse precision matrix $\vec{Q}$ is used to model dependence between the elements of $\vec{\eta}$. 
...
%
%
% To cater for irregularly spaced basis functions, \pkg{FRK} v2 also offers a sparse precision matrix which considers the Euclidean distance between basis functions.
...
\]
Recall that $D = \cup_{i = 1}^N A_i$, where $\{A_i\}$ are non-overlapping BAUs. 
...
% \pkg{FRK} v2 is thus backward compatible: An identity link function and a Gaussian data model yields the model used in \pkg{FRK} v1.
...
Given $m$ observations with (possibly overlapping) spatial support defined on one or more BAUs, we write the observation supports as $B_j \equiv \cup_{i\in c_j} A_i$, for $j = 1, \dots, m$, where $c_j$ is a non-empty set in the power set of $\{1, \dots, N\}$ that gives the indices of the BAUs associated with observation $j$. 
 ...
 The vector of observations (the data vector) is then $\vec{Z} \equiv \left(Z_1, \dots, Z_m\right)^\tp$, where $Z_j \equiv Z(B_j)$, for $j = 1, \dots, m$. 
  In practice, $B_j$  may not include entire BAUs; in this case, we assume that an areal spatial support contains a BAU if and only if there is at least some overlap between the BAU and the spatial support.
\pkg{FRK} v1 assumed that a spatial support contains a BAU if and only if the BAU centroid lies within the region; the relaxed condition in \pkg{FRK} v2 is intended to cater for non-convex BAUs, such as those used in Section \ref{sec:spatialCOS}. 
 Of course, attributing point-referenced data to BAUs is straightforward. 
...
% \node at (1.5,4.5) {$D^O$};
\draw[step=1cm] (0,0) grid (3,4);
\filldraw[color=red, fill=red, opacity = 0.7] (0.25, 3.75) rectangle (0.9, 2.25);
...
%\end{equation}
\begin{equation}
\vec{C}_Z \equiv \Big(w_{i, j}\mathbb{I}(i \in c_j) : i = 1, \dots, N; j = 1, \dots, m\Big),
\end{equation} 
...
In \pkg{FRK} v2, the weights $w_{i, j}$ may be controlled through the \code{wts} field of the BAU object and the argument \mbox{\code{normalise\_wts}}. 
 The \code{wts} field allows one to attribute each BAU with some proportionality constant, $\{v_i: i = 1, \dots, N\}$, such that $w_{i, j} \propto v_i$.  
 For example, if the BAUs are of unequal area, then one may wish to set $v_i = |A_i|$. 
 By default (and implicit in \pkg{FRK} v1), each $v_i$ is set to 1, so that $w_{i, j}$ is constant over a given $j$. 
 The argument \mbox{\code{normalise\_wts}} controls whether $\vec{C}_Z$ corresponds to a weighted sum or a weighted average; if it is \mbox{\code{TRUE}} (default, and implicit in \pkg{FRK} v1), then the weights $w_{i, j}$ are normalised so that each row of $\vec{C}_Z$ sums to 1, and the mapping represents a weighted average. 
 Note that $w_{i, j} = |A_i|/|B_j|$, a ubiquitous case in spatial statistics, is accommodated by setting $v_i = |A_i|$ and \mbox{\code{normalise\_wts = TRUE}}. 
 This exploits the fact that the normalisation process, whereby each $w_{i, j}$ is divided by $\sum_{i \in c_j} w_{i, j}$ (the $j$th row sum of $\vec{C}_Z$), cancels out any term that is constant over a given $j$. 
%  To see why this is so, consider the $j$th row sum of $\vec{C}_Z$, $\sigma_j \equiv \sum_{i \in c_j} w_{i, j}$. If $w_{i, j} = |A_i|/|B_j|$, then $\sigma_j = |B_j|^{-1}\sum_{i \in c_j} |A_i|$, and the normalised weights are 
%  \[
%  \tilde{w}_{i, j} = w_{i, j} / \sigma_j = (|A_i||B_j|^{-1}) / (|B_j|^{-1}\sum_{i \in c_j} |A_i|) = |A_i| / \sum_{i \in c_j} |A_i|.
%  \] 
...
Denoting the $j$th element of $\vec{\mu}_Z$ by $\{\vec{\mu}_{Z}\}_j$, we assume that 
\begin{equation}\label{eqn:Z_j|mu(.)}
[Z_j \mid \mu(\cdot), \psi] = \text{EF}(\{\vec{\mu}_{Z}\}_j, \psi),
\end{equation}
where EF corresponds to a probability distribution in the exponential family with dispersion parameter $\psi$, and, for generic random quantities $A$ and $B$, $[A \mid B]$ denotes the probability distribution of $A$ given $B$. 
 We assume that $\psi$ is spatially invariant, but note that $\psi = 1$ for some distributions in the exponential family (e.g., binomial, negative-binomial, and Poisson distributions). 
...
\[
[\vec{Z} \mid \mu(\cdot), \psi] = [\vec{Z} \mid \vec{\mu_Z}, \psi] = \prod_{j=1}^m \text{EF}(\{\vec{\mu}_{Z}\}_j, \psi).
\]
Further, as we only consider data models in the exponential family, $\ln{[\vec{Z}  \mid  \vec{\mu}_Z, \psi]}$ may be expressed as  
\begin{equation}\label{eqn:ln[Z|Y],ExpFam}
\ln{[\vec{Z} \mid \vec{\mu}_Z, \psi]}
=
\sum_{j=1}^m\left\{
\frac{Z_j\lambda(\{\vec{\mu}_{Z}\}_j) - b(\lambda(\{\vec{\mu}_{Z}\}_j))}{a(\psi)} + c(Z_j, \psi)\right\},
\end{equation}
...
%We model the conditional distribution $[Z_j \mid \{\vec{\mu}_{Z}\}_j, \psi]$ as a member of the exponential family \citep[Sec.~2.2.2]{McCullagh_Nelder_1989_GLM}, with conditional expectation $\mu(B_j) \equiv \ECurly{Z_j \mid \{\vec{\mu}_{Z}\}_j, \psi}$. 
 

The model employed by \pkg{FRK} v2 can be summarised as follows. 
\begin{gather}
    Z_j \mid \{\vec{\mu}_{Z}\}_j, \psi \inddist \text{EF}(\{\vec{\mu}_{Z}\}_j, \psi), \quad j = 1, \dots, m, \label{eqn:new_model_Z}\\
...
%from Section \ref{subsection:02-03:Estimation} 
 that the Laplace approximation implies that, approximately, $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$.  
 Since $\vec{Y}$ is a linear function of $\vec{u}$,  inference on $Y(\cdot)$ can hence be done using closed form solutions. 
  However, the posterior distribution of non-linear functions of $Y(\cdot)$ (e.g., the mean process) are typically not available in closed form, and some type of approximation is required.
   Hence, we choose to use a MC framework.
...
%\]
Like before, the weights $\{\tilde{w}_{i,k}\}$ are controlled by the \code{wts} field of the BAU object and the argument \mbox{\code{normalise\_wts}}. 
 MC samples of $\vec{\mu}_P \mid \vec{Z}, \vec{\theta}$ can be constructed  via $\vec{M}_P \equiv \vec{C}_P \vec{M}$, where recall that $\vec{M}$ consists of MC samples of $\vec{\mu} \mid \vec{Z}, \vec{\theta}$. Predictions and uncertainty quantification of those predictions can then be computed straightforwardly. 
...
We now demonstrate the new features in \pkg{FRK} v2, an overview of which is presented in Table \ref{tab:new_arguments_in_functions}. 
 ...
 In Section \ref{sec:03-01:Poisson}, we illustrate use of \pkg{FRK} v2 using non-Gaussian spatial point-referenced data. %Further code examples are given in Section \ref{SEC:ApplicationStudy}.%, whilst in Section \ref{sec:03-03:negative-binomial} we discuss non-Gaussian  spatial areal data and spatial change-of-support. 
% In Section \ref{sec:spatial_COS_and_ST}, we briefly discuss extensions available in \pkg{FRK} v2 to model areal data and data in a spatio-temporal setting. 
 ...
 Finally, in Section \ref{sec:3:increased_resolution}, we show the potential improvement in predictive performance of \pkg{FRK} v2 over \pkg{FRK} v1, thanks to the support for an increased number of basis functions in \pkg{FRK} v2.
% 
...
%        & \code{k} & Vector of size parameters at each prediction location (applicable only for binomial and negative-binomial data).\\
        & \code{n\_MC} & The number of MC samples at each BAU.\\
...
The first step when using \pkg{FRK} v1/v2 is to create basis functions and BAUs, a task that can be performed automatically using the helper functions \fct{auto\_BAUs} and \fct{auto\_basis}: See \cite{FRK_paper} for details. 
...
The second element is a list of matrices containing MC samples of the quantities of interest at each prediction location. 
...
In this section, we illustrate \pkg{FRK} v2 on non-Gaussian, \textit{areally}-referenced, spatial data, as well as its use in predicting over large areas. 
...
%Another new feature in \pkg{FRK} v2 is the option to provide a known fine-scale variance parameter through the argument \code{known\_sigma2fs}.
...
Although the primary motivation for \pkg{FRK} v2 is the provision of non-Gaussian data models, the ability to use  a large number of basis functions also leads to higher-quality predictions in a Gaussian setting.
...
To achieve these improvements over \pkg{FRK} v1, we only had to specify \code{nres = 4} in the \fct{auto\_basis} function, and then \code{K\_type = "precision"} and \code{method = "TMB"} in the \fct{SRE} and \fct{SRE.fit} functions, respectively; the rest of the \pkg{FRK} code as used in the competition was left unchanged. 
...
%\begin{gather}
%    Z_j \mid \{\vec{\mu}_{Z}\}_j \inddist \text{EF}(\{\vec{\mu}_{Z}\}_j), \quad j = 1, \dots, m,  \label{eqn:new_model2_Z}\\
