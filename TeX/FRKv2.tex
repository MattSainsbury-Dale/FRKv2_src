...
\Abstract{
 ...
 Finally, although the primary motivation for this major upgrade is the modelling of non-Gaussian data, this new version of \pkg{FRK} also allows for the use of substantially more basis functions than previous versions when modelling the spatial process and, hence, in a Gaussian setting, it is often able to achieve more accurate predictions than \pkg{FRK} v1. 
...
$\vec{\sigma}^2_\xi \equiv (\sigma^2_{\xi, 1}, \dots, \sigma^2_{\xi, N_s})^\tp$, 
$\text{vec}(\cdot)$ `stacks' its arguments into a single vector, $\text{diag}(\cdot)$ returns a diagonal matrix from a vector argument, and $\odot$ denotes element-wise multiplication. 
 This model is flagged by setting \mbox{\code{fs\_by\_spatial\_BAU = TRUE}} in the \fct{SRE} function. 
  This model may be advantageous when the variance of the fine-scale component is spatially varying, and it is particularly useful when the number of spatial BAUs (and hence the number of variance parameters to estimate) is relatively low, and when we have observations from each spatial BAU at many time-points; see, for instance, the Chicago example presented in Section \ref{sec:ST_example}. 
...
Michael Bertolacci for his discussion surrounding the MODIS comparison study, 
and Yi Cao for his technical assistance during the project and for his assistance in reviewing reproducibility aspects of the scripts associated with the paper.  
