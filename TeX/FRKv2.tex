...
%  Attributing point-referenced data to BAUs is straightforward.
  We then define the set of observation supports \red{in terms of BAUs} as $D^O \equiv \{B_j : j = 1, \dots, m\} $, where $B_j \equiv \cup_{i\in c_j} A_i$ is the representation of $R_j$ \red{in terms of BAUs}\footnote{\red{Noel; I wonder if there is some redundancy in the repetition of `in terms of BAUs'?}}. 
...
In \pkg{FRK} v2, the weights $w_{ij}$ \red{may be controlled through the argument \mbox{\code{normalise\_wts}} and the \code{wts} field of the \class{SpatialPixelsDataFrame}/\class{SpatialPolygonsDataFrame} object \citep{Pebesma_2005_sp_package} used to store the BAUs.} 
 ...
 By default (and implicit in \pkg{FRK} v1), each $v_i$ is set to 1. 
  The argument \mbox{\code{normalise\_wts}} controls whether $\vec{C}_Z$ corresponds to a weighted sum or a weighted average. If set to \code{FALSE}, then $w_{ij} = v_i$ for all $j$ (weighted sum); if set to \mbox{\code{TRUE}} (default and implicit in \pkg{FRK} v1), then the $\{w_{ij}\}$ are normalised so that each row of $\vec{C}_Z$ sums to 1 (weighted average) and $w_{ij} = v_i / \sum_{k \in c_j} v_l$.   
...
\end{gather}
 ...
 Note that \pkg{FRK} v2 is backwards compatible, since an identity link function and a Gaussian data model in (\ref{eqn:new_model_Z}), where $\psi$ plays the role of the measurement-error variance, \red{$\var{Z_j \mid Y_j}$ that does not depend on $j$}\footnote{\red{Noel, I just wanted to make sure that this sentence is grammatically correct}}, yields the model used in \pkg{FRK} v1.
...
where every column of the matrix $\vec{A}$ is the estimated posterior mode of $\vec{\alpha}$, and each of the $n_{\text{MC}}$ columns of the matrix $\vec{U}$ are draws from  $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$.  
 ...
 Note that \pkg{FRK} v2 provides the user with $\vec{M}$ which, if needed, can be used to predict data with link function \red{$g(\cdot)$}\footnote{Noel, you had written $g$ instead of $g(\cdot)$. I prefer the latter for consistency with the rest of the manuscript, but perhaps there is a reason to omit the parentheses?} but from a distribution that is different from that of the original data.
...
\subsubsection{Arbitrary prediction regions}
 ...
 We then define the set of prediction regions \red{in terms of BAUs} as $D^P \equiv \{\tilde{B}_l : l = 1, \dots, N_P\}$, where $\tilde{B}_l \equiv \cup_{i\in \tilde{c}_l} A_i$ is the representation of $\tilde{R}_l$ \red{in terms of BAUs}. %, and where $\tilde{c}_l$ is a non-empty subset of $\{1, \dots, N\}$ that gives the indices of the BAUs associated with the $k$th user-specified region, $\tilde{R}_l$.
...
\subsection{Distributions with size parameters}\label{sec:Distributions with size parameters}
 ...
 \green{Software that cater for these distributions typically allow `link' functions such as the logit, probit, and complementary log-log functions.  
 In \pkg{FRK} v2, we use these functions to link the latent spatial process, $Y(\cdot)$, to a probability process, $\pi(\cdot)$: 
...
When modelling negative-binomial data, other popular link functions include the log and square-root functions. 
 ...
 \green{
 Regardless of the chosen link function, we define the probability process evaluated over the observation supports, $B_j$, $j = 1, \dots, m$, as $\vec{\pi}_Z \equiv (\pi_{Z_j} : j = 1, \dots, m)^\tp$, where
\begin{equation}
...
%\end{equation}
 %% Vector version (avoids problems associated with subscripting)
  Now define the vector of size parameters associated with the prediction regions, $\tilde{B}_l$, $l = 1, \dots, N_P$, as $\vec{k}_P \equiv (\sum_{i \in \tilde{c}_l} k_i : l = 1, \dots, N_P)^\tp$, where recall that $\tilde{c}_l$ denotes the indices of the BAUs associated with the prediction region $\tilde{B}_l$: Then the probability process evaluated over the prediction regions is 
  \begin{equation}
  \vec{\pi}_P \equiv h(\vec{\mu}_P; \vec{k}_P),
  \end{equation} 
  where $h(\cdot\,; \cdot)$ is applied elementwise. % and $\vec{k}_P \equiv \vec{C}_P \vec{k}$. 
}

 \green{When model fitting, if each observation is associated with exactly one BAU, the user can provide the data-level size parameters $\vec{k}_Z$ only, which are used to infer the BAU-level size parameters; in particular, $k_i = \sum_{j \in a_i} k_{Z_j}$, $i = 1, \dots, N$, where $a_i$ denotes the indices of the observations associated with BAU $A_i$.}  
 In the case where one or more observations encompass multiple BAUs, size parameters \red{must be} set at the BAU level: Data size parameters are then selected from the associated BAU-level size parameters, \mbox{$\{k_i : i = 1, \dots, N\}$}, and summed; that is, $k_{Z_j} = \sum_{i \in c_j} k_i$, $j = 1, \dots, m$. %or, equivalently, $\vec{k}_Z = \vec{C}_Z \vec{k}$. 
...
See the package documentation for details.
 ...
 Therefore, with these distributions, \pkg{FRK} v2 \red{enforces} the matrix $\vec{C}_Z$ in (\ref{eqn:C_Z}) \red{to be} constructed with the relative weights \mbox{$\{v_i = 1$ : $i = 1, \dots, N\}$} and with \mbox{\code{normalise\_wts = FALSE}}, and hence \red{$w_{ij} = 1$} in (\ref{eqn:C_Z}). 
 ...
 \green{Since \pkg{FRK} v2 enforces the use of the same relative weights, $\tilde{v}_i = v_i$ for $i = 1, \dots, N$, and the same setting of \code{normalise\_wts}, in construction of both $\vec{C}_Z$ and $\vec{C}_P$, the mapping from $\vec{\mu}$ to $\vec{\mu}_P$ in (\ref{eqn:mu_P}) is also a simple, unweighted summation over the BAUs. }
...
\subsection{Example: Non-Gaussian, point-referenced spatial data}\label{sec:03-01:Poisson}
 ...
 In what follows,
%  \red{$\text{Poi}(\mu)$ refers to a probability distribution on the non-negative integers with probability mass function, $P(Z = z) = e^{-\mu} \mu^z/z!$ for $z = 0, 1, \dots$.}\footnote{In what follows, \red{$\text{Poi}(\lambda)$ refers to a probability distribution on the non-negative integers, namely $p_j = e^{-\lambda} \lambda^j/j!$ for $j = 0, 1, \dots$.}}
 \red{$\text{Poi}(\mu)$ refers to a probability distribution on the non-negative integers with probability mass function (PMF) $\{p_z\}$, namely $p_z = e^{-\mu} \mu^z/z!$ for $z = 0, 1, \dots$.}\footnote{\red{Noel, I ended up trying the $p_z$ notation; I like its visual simplicity and I like that we don't need to write $Z \mid \mu \sim$ before the distribution. However, I’m happy to change it to $P(Z = z)$ if you prefer.}}
...
Next, an \class{SRE} object is constructed using \fct{SRE}, within which we specify the data model, the link function, and the parameterisation of $\cov{\vec{\eta}}{\vec{\eta}}$.  
 In this example, we model counts using a Poisson data model, $Z_j \mid \vec{\mu} \sim \text{Poi}(\mu_{Z_j})$ \red{at given observation support $B_j$}\footnote{\red{Noel, do you think this is necessary? $Z_j$ and $\mu_{Z_j}$ are implicitly linked with $B_j$. If you think this phrase helps, then perhaps I should also add it in later sections when I define the data model?}}, $j = 1, \dots, m$, and we use the log link function, $g(\cdot) = \log(\cdot)$. 
...
In this section, we illustrate \pkg{FRK} v2 on simulated negative-binomial, areally-referenced, spatial data, as well as its use in predicting over areas with large spatial supports. 
  \red{In what follows, $\text{NB}(k, \pi)$ refers to a distribution on the non-negative integers with PMF $\{p_z\}$, namely \mbox{$p_z = {{z + k - 1}\choose z}\pi^k(1-\pi)^z$} for $z = 0, 1, \dots$ and $k = 1, 2, \dots$.}
  %  \red{Here, $\text{NB}(k, \pi)$ denotes a random variable from a probability distribution on the non-negative integers with probability mass function, \mbox{$P(Z = z) = {{z + k - 1}\choose z}\pi^k(1-\pi)^z$} for $z = 0, 1, \dots$.}\footnote{\red{Here, $\text{NB}(\lambda, k)$ is a probability distribution on the non-negative integers, namely $p_j = {{j + k - 1}\choose j}\lambda^k(1-\lambda)^j$ for $j = 0, 1, \dots$.}}
...
We then construct the mean process evaluated over the BAUs, $\vec{\mu}$, which is shown in the centre-left panel of Figure \ref{fig:03-02-negative-binomial_data}; as we are simulating negative-binomial data, this requires specification of assumed-known size parameters \red{representing the target number of successes}, and, for simplicity, we set $k_i = 50$, $i = 1, \dots, N$.  
 ...
 \green{Note that the fine-scale elements of $\vec{\mu}_Z$ are small in comparison to the coarse-scale elements: When the response distribution is binomial or negative-binomial, the mapping from $\vec{\mu}$ to $\vec{\mu}_Z$ in (\ref{eqn:04-01:mu_Z}) is a simple, unweighted summation over the BAUs, and hence $\mu_{Z_j}$ is proportional to the size of the observation support $B_j$.} 
 Finally, we simulate negative-binomial data, \red{\mbox{$Z_j \mid \vec{\mu}, \vec{k}_Z \sim \text{NB}(k_{Z_j}, \pi_{Z_j})$}}, $j = 1, \dots, m$, \red{which represent the number of failures before the target number of successes, $k_{Z_j}$; these data are} shown in the right panel of Figure \ref{fig:03-02-negative-binomial_data}. 
...
\begin{figure}[t!]
    ...
    Shown in white are spatial supports (at both fine-resolution and coarser-resolution) where there are no observations.
}   
  \label{fig:03-02-negative-binomial_data}
\end{figure}

 Now we construct and fit the \class{SRE} object using \fct{FRK}. 
 In this example, the data model is negative-binomial, that is, \red{$Z_j \mid \vec{\mu}, \vec{k}_Z \sim \text{NB}(\pi_{Z_j}, k_{Z_j})$}, $j = 1, \dots, m$, and we use the logit link function, \red{$f(\pi) = \log(\frac{\pi}{1 - \pi})$}. 
...
\begin{figure}[t!]
    ...
    (Left panel) Prediction of the probability process over the BAUs, $\vec{\pi}$. 
    (Centre-left panel) Width of the 90\% prediction interval for each element of $\vec{\pi}$. 
    (Centre-right panel) Prediction of the mean process over the BAUs, $\vec{\mu}$. 
...
In contrast, the prediction-interval width for $\vec{\pi}$ is low when the prediction is near 0 or 1, and it increases when the prediction is near 0.5: This is expected from properties of the negative-binomial distribution. 
 ...
 The mean empirical coverage from the 90\% prediction intervals was 90.9\%, which is very close to the nominal value.

To illustrate that user-specifed prediction regions, $\tilde{R}_l$, $l = 1, \dots, N_P$, are arbitrary, we demonstrate prediction over a handful of irregularly-shaped \red{polygons}. % (defined as a \class{SpatialPolygons} object).

\begin{Code}
R> pred <- predict(S, newdata = arbitrary_polygons)
\end{Code}

Figure \ref{fig:03-02-negative-binomial_polygons} shows the predictions and prediction-interval widths, respectively, for the \green{probability process, $\vec{\pi}_P$, and the mean process, $\vec{\mu}_P$, when predicting over arbitrary prediction regions, $\tilde{R}_l$, $l = 1, \dots, N_P$.}  

\begin{figure}[t!]
    \centering
    \includegraphics[width = \linewidth]{img/Negbinom_sim_arbitrary_polygon_predictions.png}
	\vspace{-20pt}    
%    \caption{\red{Prediction (left panel) and prediction-interval widths (right) for the aggregated mean, $\vec{\mu}_P$, when predicting over arbitrary prediction regions, $\tilde{R}_l$, $l = 1, \dots, N_P$, in the negative-binomial example of Section \ref{sec:03-03:negative-binomial}. Note that these polygons have equal area.}}
    \caption{\green{Predictions and prediction-interval widths for the probability process, $\vec{\pi}_P$, and the mean process, $\vec{\mu}_P$, when predicting over arbitrary prediction regions, $\tilde{R}_l$, $l = 1, \dots, N_P$, in the negative-binomial example of Section \ref{sec:03-03:negative-binomial}. Note that these polygons have equal area. 
    (Left panel) Prediction of the probability process over the prediction regions, $\vec{\pi}_P$. 
    (Centre-left panel) Width of the 90\% prediction interval for each element of $\vec{\pi}_P$. 
    (Centre-right panel) Prediction of the mean process over the prediction regions, $\vec{\mu}_P$. 
...
\begin{table}[t!]
    ...
    \caption{\red{Scores for each method in the comparative study presented in \cite{Heaton_2019_comparative_study}.}}
...
%  \red{In what follows, $\text{Bin}(k, \pi)$ refers to a probability distribution on the integers $\{0, 1, \dots, k\}$ with probability mass function, $P(Z = z) = {k \choose z}\pi^z(1-\pi)^{k-z}$.}
  \red{In what follows, $\text{Bin}(k, \pi)$ refers to a probability distribution on the non-negative integers with PMF $\{p_z\}$, namely \mbox{$p_z = {k \choose z}\pi^z(1-\pi)^{k-z}$} for $k = 1, 2, \dots$ and $z = 0, 1, \dots, k$. For the special case of $k = 1$, $\text{Bin}(1, \pi)$ is the Bernoulli distribution.}
...
\end{table}
 ...
 The main advantages of \pkg{FRK} v2 however lie in the ease with which it allows one to do other more elaborate analyses with spatial or spatio-temporal non-Gaussian data of differing support, as shown in the next sections. 
...
% Note that \pkg{FRK} v2 does not explicitly cater for a lognormal response, but an effective approach is to use \code{response = "Gaussian"} and \code{link = "log"}.  
 ...
 These blocking schemes represent the user-specified prediction regions, $\tilde{R}_l$, $l = 1, \dots, 10$. 
...
It is often the case that sampling once from a large area is relatively inexpensive compared to acquiring multiple samples from small areas. 
 ...
 \red{SA2 regions are aggregations of SA1 regions, so some of the observation supports, $B_j$, encompass multiple BAUs and the BAU-level size parameters, $\vec{k}$, which in this case represent the total number of families in each SA1 region, must be provided with the BAU object.}  
 \red{Our data model is $Z_j \mid \vec{\mu}, \vec{k}_Z \sim \text{Bin}(k_{Z_j}, \pi_{Z_j})$, $j = 1, \dots, m$,} and we use the logit link function, $f(\pi) = \log(\frac{\pi}{1 - \pi})$. 
\begin{Code}
...
% Since the SA1 regions are our BAUs, we can predict both the probability process, $\vec{\pi}$, and the mean process, $\vec{\mu}$, over the SA1 regions: 
 \red{We focus on $\vec{\pi}$, which can be interpreted as the proportion of families living below the poverty line, although we could also have focused on $\vec{\mu}$. } 
...
% To predict over the SA3 regions, we simply set \code{newdata} to a  \class{SpatialPolygonsDataFrame} object containing the SA3 regions.
 ...
 \begin{figure}[t!]
    ...
    \includegraphics[width = \linewidth]{img/Sydney_SA1_predictions_mixed.png}
        \vspace{-15pt}
    \caption{SA1-level predictions. (Left panel) Prediction of the probability process over the SA1 regions, $\vec{\pi}$, representing the proportion of families in each region. (Right panel) 90\% prediction-interval width for each element of $\vec{\pi}$.
...
\end{Code}
 Figure \ref{fig:SA3_predictions} shows the SA3-region predictions and associated prediction-interval widths for the mean process, $\vec{\mu}_P$. 
...
\begin{figure}[t!]
    ...
    \includegraphics[width = \linewidth]{img/Sydney_SA3_predictions_probability_mixed.png}
        \vspace{-15pt}
    \caption{SA3-level predictions. (Left panel) Prediction of the probability process over the SA3 regions, $\vec{\pi}_P$, representing the 
    \red{proportion} 
    of families in poverty in each region. 
...
Then, we initialise and fit the \class{SRE} object using \fct{FRK}, setting \code{response = "poisson"} and \code{link = "log"} in order to use a Poisson data model, $Z_j \mid \vec{\mu} \sim \text{Poi}(\mu_{Z_j})$, $j = 1, \dots, m$, and a log link function, $g(\cdot) = \log(\cdot)$. 
...
% is the vector of BAU-level size parameters. 
 ...
 \green{Recall from Section \ref{sec:Distributions with size parameters} that $h(\cdot\,; \cdot)$ is a function used in (\ref{eqn:h_mu_equals_pi}) to link the BAU-level probability process, $\vec{\pi}$, to the BAU-level mean process, $\vec{\mu}$. 
