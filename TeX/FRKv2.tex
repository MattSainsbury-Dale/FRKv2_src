...
The existing functionality of \pkg{FRK} is retained with this advance into non-linear, non-Gaussian models; in particular, it makes use of automatic basis-function construction, it can handle both point-referenced and areal data simultaneously, and it predicts process values at any spatial support from these data. 
 These nvastly more general spatial and spatio-temporal models are fitted by using the Laplace approximation and the software \pkg{TMB} to obtain likelihood-based estimates. 
We demonstrate innovative features in this version of \pkg{FRK} and compare it to alternative packages, using both simulated and real data sets. 
}
...
Several other approaches hinge on the use of a spatial version of the generalised linear mixed model \citep[GLMM;][]{Diggle_1998_spatial_GLMM}, whereby the response distribution is assumed to be a member of the exponential family of distributions \citep{McCullagh_Nelder_1989_GLM}, and the conditional mean is modelled using a transformation of some latent spatial process $Y(\cdot)$. 
...
\citet{Bradley_2018_computationally_efficient_multivariate_ST_models_for_high-dimensional_count-valued_data}, \citet{Bradley_2019_ST_models_for_big_multinomial_data_using_conditional_multivariate_logit_beta_distribution}, and \citet{Zhang_2020_spatio-temporal_Arctic_sea_ice}. 
 ...
 The main purpose of this article is to present a major advance that caters for many distributions within the exponential family using the spatial GLMM framework; we henceforth refer to it as \pkg{FRK} v2. 
 It provides a unifying framework that handles very large, spatial and spatio-temporal non-Gaussian data, and it seamlessly ingests point-referenced and area-referenced data to solve spatial change-of-support problems. %facilitates change-of-support over target supports that may be smaller or larger than the source supports (i.e., it caters for statistical down-scaling and up-scaling). 
  User-friendliness is a central focus of the package: Challenging statistical problems may be tackled with only several lines of intuitive, readable code. Optimal spatial prediction proceeds through the use of an \textit{empirical} hierarchical statistical model (where likelihood-based estimates are substituted in place of unknown parameters) and a Monte Carlo (MC) algorithm, where a minimal number of user-level decisions is required.
...
The statistical model used in \pkg{FRK} v2 is a spatial or spatio-temporal GLMM, a hierarchical statistical model consisting of two conditional-probability layers.
...
Accounting for only large and medium-to-small-scale spatial variation can result in overly smooth prediction surfaces, which in turn may lead to overly optimistic predictions; this problem is alleviated by including a fine-scale-variation random process, $\xi(\cdot)$, that is `almost' uncorrelated.
...
See \cite{FRK_paper} for details on how these basis functions are constructed. 
 The fine-scale term, $\xi(\cdot) \equiv \{\xi(\vec{s}): \vec{s} \in D\}$, is modelled as a white-noise process.
...
BAUs are a key element of \pkg{FRK} v1/v2, as they provide a framework that allows one to use both point-referenced and areal data simultaneously, and one that facilitates solutions to spatial change-of-support problems. 
 Define the latent spatial process $Y(\cdot)$ evaluated over the $N$ BAUs as 
 \[
...
As in \pkg{FRK} v1, the elements of $\vec{\xi}$ are modelled as independent and identically distributed (i.i.d.) Gaussian random variables with variance $\sigma^2_\xi$, and $\vec{\eta}$ is modelled as a mean-zero multivariate-Gaussian random variable with covariance matrix $\cov{\vec{\eta}}{\vec{\eta}}$. 
 ...
 Both formulations use block-diagonal matrices, so that basis-function coefficients between resolutions are independent; see Appendix \ref{Appendix:CovarianceTapering} for details on the intra-resolution dependencies. 
...
Given $m$ observations with (possibly overlapping) spatial support defined on one or more BAUs, we write the observation supports as $B_j \equiv \cup_{i\in c_j} A_i$, for $j = 1, \dots, m$, where $c_j$ is a non-empty set in the power set of $\{1, \dots, N\}$ that gives the indices of the BAUs associated with observation $j$. 
 ...
 The vector of observations (the data vector) is then $\vec{Z} \equiv \left(Z_1, \dots, Z_m\right)^\tp$, where $Z_j \equiv Z(B_j)$, for $j = 1, \dots, m$. 
  In practice, $B_j$  may not include entire BAUs; in this case, we assume that an areal spatial support contains a BAU if and only if there is non-empty intersection between the BAU and the spatial support.
...
\end{scope}
\end{tikzpicture}
\caption{A simple example demonstrating how the continuous spatial domain, $D$, is discretised into the set $D^G$ of  BAUs, and how the observation domain, $D^O$, is derived from the observation supports.
...
Define the mean at the observation level as $\vec{\mu}_Z \equiv (\E{Z_1}, \dots, \E{Z_m})^\tp$. Then, since each $B_j \in D^O$ is either a BAU or a union of BAUs, one can construct an $m\times N$ matrix 
%\begin{equation}
...
Denoting the mean of $Z_j$, which is the $j$th element of $\vec{\mu}_Z$, by $\vec{\mu}_{Zj}$, we assume that 
...
Equation (\ref{eqn:Z_j|mu(.)}) implies that a given observation depends only on the value of the mean process at the corresponding observation support, rather than on means from the whole domain. 
Further, we assume that all observations are conditionally independent given the latent spatial process:
\[
[\vec{Z} \mid \vec{\mu}_Z, \psi] = \prod_{j=1}^m \text{EF}(\vec{\mu}_{Zj}, \psi).
\]
...
\end{equation*}
 ...
 yields the Laplace approximation of the observed-data likelihood, $L^*(\vec{\theta}; \vec{Z})
...
%Using a second-order Taylor series approximation of $l(\vec{\theta} ; \vec{Z}, \vec{u})$ about $\red{\vec{u} =} \hat{\vec{u}}$, the Laplace approximation of the observed-data likelihood is
%\begin{align*}
%    L^*(\vec{\theta}; \vec{Z})
%    \approx L(\vec{\theta} ; \vec{Z}, \hat{\vec{u}})
%    (2\pi)^{\frac{{p}}{2}}\left|\vec{H}\right|^{\frac{1}{2}},
%\end{align*}
%and so the Laplace approximation of the observed-data log likelihood is 
...
Given a \proglang{C++} template function that defines $l(\vec{\theta} ; \vec{Z}, \vec{u})$, \pkg{TMB} \citep{Kristensen_2016_TMB} computes the Laplace approximation of the observed-data log-likelihood, and it automatically computes its derivatives. These quantities are then called from within \pkg{FRK} v2 by an optimising function specified by the user (\fct{nlminb} is used by default). 
 \pkg{TMB} uses \pkg{CppAD} \citep{CppAD_Package} for automatic differentiation, and it uses the linear algebra libraries \pkg{Eigen} \citep{Eigen} and \pkg{Matrix} \citep{Matrix_Package} for vector and matrix operations in \proglang{C++} and \proglang{R}, respectively. Use of these packages yields high computational efficiency. 
\pkg{TMB}'s implementation of automatic differentiation is a key reason why \pkg{FRK} v2 can cater for a variety of response distributions combined with a variety of link functions, as each combination does not need to be considered on a case-by-case basis.
...
We fix the parameters and fixed effects to their posterior-mode estimates, and then they are treated as non-random quantities.
...
%For each quantity, we use the posterior expectation as our predictor. % ; a decision theoretic justification for this choice is provided by \citet*[ch.~3]{Cressie_1993_stats_for_spatial_data}.
Recall that the Laplace approximation implies that approximately, $\vec{u} \mid \vec{Z}, \vec{\theta} \sim \Gau(\hat{\vec{u}}, \vec{H})$; since $\vec{Y}$ is a linear function of $\vec{u}$, inference on $Y(\cdot)$ can hence be done using closed form solutions. 
  However, the posterior distribution of non-linear functions of $Y(\cdot)$ (e.g., the mean process) are typically not available in closed form, and some type of approximation is required.
   We choose to use a Monte Carlo (MC) framework.
...
$D^P \equiv \{\tilde{B}_k : k = 1, \dots, N_P\}$, where $\tilde{B}_k \equiv \cup_{i\in \tilde{c}_k} A_i$, and where $\tilde{c}_k$ is some non-empty set in the power set of $\{1, \dots, N\}$. 
...
Prediction over the prediction regions, $D^P$, requires some form of aggregation across relevant BAUs.
...
We model the $r_tr_s \times r_tr_s$ precision matrix  of the $r_tr_s$ spatio-temporal random coefficients associated with basis functions created through the tensor product of the $r_t$ temporal basis functions and the $r_s$ spatial basis functions as
\[
\vec{Q} = \vec{Q}_t \otimes \vec{Q}_s.
\]
The assumption of space-time separability of the precision matrix $\vec{Q}$ and hence the ability to write it as a Kronecker product, leads to significant computational savings. 
...
$\vec{\xi} \sim \Gau\left(\vec{0}, \vec{\Sigma}_\xi\right)$, where the $N \times N$ matrix, 
\begin{equation}
...
\end{pmatrix},
\end{equation}
such that $\vec{\sigma}^2_\xi \equiv (\sigma^2_{\xi, 1}, \dots, \sigma^2_{\xi, N_s})^\tp$,
and the BAUs are assumed to be ordered such that space runs faster than time. 
This model for $\vec{\Sigma}_\xi$ is flagged by setting \code{fs\_by\_spatial\_BAU = TRUE} in the \fct{SRE} function. It is particularly useful when the number of spatial BAUs (and hence variance parameters to estimate) is relatively low and we have observed each spatial BAU over many time-points; see, for instance, the example presented in Section \ref{sec:ST_example}. 
...
\begin{table}
 ...
 \caption{Important extensions to function arguments in \pkg{FRK} v2.}\label{tab:new_arguments_in_functions} 
    ...
    \fct{SRE} & \code{response}  & A string indicating the data model. \\
        ...
        \\
        & \code{normalise\_wts} & A flag controlling whether the weights in $\vec{C}_Z$ and $\vec{C}_P$ should be normalised.\\
...
%        & \code{taper}     & A positive scalar indicating the strength of the covariance tapering (see Appendix \ref{Appendix:CovarianceTapering}).\\
        & \code{known\_sigma2fs} & Allows the fine-scale variance to be fixed to a known value. 
...
%        If \code{"link"} is in \code{type}, $Y(\cdot)$ is included; if \code{"mean"} is in \code{type},  the mean process (and the probability process, if applicable) is included; if \code{"response"} is in \code{type},  the response variable is included. 
        The inclusion of \code{"link"}, \code{"mean"}, and \code{"response"} in \code{type}, respectively indicate that inference on $Y(\cdot)$, $\mu(\cdot)$, or the noisy data is desired.\\ 
        & \code{percentiles} & Numeric vector %(each element between 0 and 100) 
        indicating the percentiles of the predictive distribution(s) to be computed.\\
...
\begin{figure}[t!]
    ...
    \caption{A simulated point-referenced spatial Poisson data set (Right panel), and the true field (Left panel) from which the data were generated; see the example presented in Section \ref{sec:03-01:Poisson}. 
}   
...
Next, an \class{SRE} object is initialised using \fct{SRE}, within which we specify the data model, the link function, and the parameterisation of $\cov{\vec{\eta}}{\vec{\eta}}$.  
 We fit the model using \fct{SRE.fit}.
 These steps may be performed in a single line of code with the convenient wrapper function \fct{FRK}. Note that when the data are non-Gaussian or when a non-identity link function is chosen, \fct{FRK} automatically selects \code{K\_type = "precision"}, which means that the basis-function coefficients are modelled via their precision matrix $\vec{Q}$. 
\begin{Code}
...
\end{Code}
The \class{ggplot} objects can then be arranged easily in a grid using various dedicated packages; we used \pkg{ggpubr} \citep{Kassambara_2020_ggpubr}. Figure \ref{fig:Poisson_nres3} shows predictions and uncertainty quantification of the predictions for the latent process $Y(\cdot)$ and the mean process $\mu(\cdot)$. 
...
\begin{figure}[t!]
    ...
    \caption{Predictions and prediction uncertainty quantifications using the data shown in Figure \ref{fig:Poisson_true_and_Z}. (Left panel) Prediction of the latent process, $Y(\cdot)$. (Centre-left panel) Width of the 90\% central prediction interval of the latent process. (Centre-right panel) Prediction of the mean process, $\mu(\cdot)$ . (Right panel) Width of the 90\% central prediction interval of the mean process. 
}   
...
In this section, we illustrate \pkg{FRK} v2 on simulated negative-binomial, areally-referenced, spatial data, as well as its use in predicting over large areas (i.e., large supports). 
...
\begin{figure}[t!]
    ...
    \caption{Prediction and uncertainty quantification of the predictions for the simulated negative-binomial areal data set. (Left panel) Prediction of the probability process $\pi(\cdot)$. (Centre-left panel) Width of the 90\% central prediction interval of the probability process. (Centre-right panel) Prediction of the mean process, $\mu(\cdot)$. (Right panel) Width of the 90\% central prediction interval of the mean process.
}   
...
In contrast, the prediction uncertainty of $\pi(\cdot)$ is low when the prediction is near 0 or 1, and it increases when the prediction is near 0.5: This is expected from properties of the negative-binomial distribution. 
...
Figure \ref{fig:03-02-negative-binomial_polygons} shows the predictions and uncertainty quantifications of the predictions over the irregularly-shaped areas. 
...
\begin{table}[t!]
    \centering
    \caption{Diagnostics comparing the predictive performance when using a range of basis-function resolutions and with point-referenced count data. The diagnostics are the root mean-square prediction error (RMPSE), the continuous ranked probability score (CRPS), and the empirical coverage (Cvg90) and interval score (IS90) resulting from a central prediction interval with a nominal coverage of 90\% (see Appendix \ref{app:ScoringRules} for further details). The diagnostics are with regard to prediction of the true mean process, $\mu(\cdot)$, and they are averaged over all unobserved locations.
...
Although the primary motivation for \pkg{FRK} v2 is the provision of non-Gaussian data models, the ability to use  a large number of basis functions also leads to better predictions in the Gaussian setting.
We show this through the comparative study published in \cite{Heaton_2019_comparative_study}.
...
The data used in that study was made up of a training and a test set consisting of 105,569 observations and 42,740 observations, respectively; see \cite{Heaton_2019_comparative_study} for a detailed description of the data.
Table \ref{tab:Heaton_comparison} replicates Table 3 of \cite{Heaton_2019_comparative_study}, with an additional entry corresponding to \pkg{FRK} v2, wherein many more basis functions are used than was practical with \pkg{FRK} v1.
 Specifically, \pkg{FRK} v1 used 485 basis functions, whilst \pkg{FRK} v2 used 12114.
...
We now provide several application studies using \pkg{FRK} v2.
...
The software used in this study each required several modelling decisions, which had to be made in a way that balanced predictive performance and run time. 
 We took a systematic approach to a pre-processing model-selection phase by splitting the training set in two, and then we used one half for model fitting and the other half for model validation. 
 ...
 We used the \fct{bam} function from \pkg{mgcv}, which is similar to generalised additive model function \fct{gam}, but it is optimised for large data sets, with 3,000 knots. 
...
Setting \code{n.omp.threads} to be greater than 1 did not work (a known issue documented in the \pkg{spNNGP} package manual); hence, our reported run-times for \pkg{spNNGP} are for a single core.
...
% Following \cite{Paul_Cressie_2011_lognormal_kriging_block_prediction}, we use two blocking schemes, one centred on GZ, and the other centred away from GZ; these schemes are shown in right panel of Figure \ref{fig:Am_data}.
 The right panel of Figure \ref{fig:Am_data} shows two blocking schemes that we predict over: Both schemes contain 5 blocks, but one scheme is centred on GZ, and the other is centred away from GZ.
% Following \cite{Paul_Cressie_2011_lognormal_kriging_block_prediction}, we use two blocking schemes, one centred on GZ, and the other centred away from GZ; these schemes are shown in right panel of Figure \ref{fig:Am_data}. 
...
The data were collected in the Census of 2011, and it consists of the number of families of various types within a range of weekly income brackets; we provide further details and the way in which we define the poverty line for each family type, in Appendix \ref{Appendix:Sydney_data_description}. 
...
\end{Code}
Figure \ref{fig:SA3_predictions} shows the SA3-region predictions and associated uncertainty quantifications: Since the SA3 regions are not the BAUs, we are restricted to prediction of the mean process. Again, this graphic was generated using \fct{plot}. 
...
\end{figure}
 ...
 If only coarse-resolution data are available (i.e., all data supports are associated with multiple BAUs), in order to avoid identifiability issues, \pkg{FRK} v2 fixes $\sigma^2_\xi$ prior to fitting the model with \pkg{TMB}. 
...
%The data considered in this analysis consisted of 1,748,360 crimes. 
 We considered only crimes labelled as assault or battery; 
% 1,748,360 
 roughly 1.75 million crimes in total. 
 Note that the default behaviour of all releases of \pkg{FRK} is to bin data falling into the same BAU; in this example, the final number of observations post-binning is significantly less than 1.75 million. 
...
Spatio-temporal BAUs may then be constructed by passing the CAs and data into \fct{auto\_BAUs}. Note that the spatio-temporal BAUs are space-time volumes constructed by taking the Kronecker product of the spatial BAUs and temporal box functions. 
\begin{Code}
...
\begin{figure}[t!]
    ...
    \caption{Observed number of crimes, predictions, and prediction uncertainty over Chicago in the prediction (2010) and forecast (2019) years. The first row corresponds to the year 2010; the second row corresponds to the year 2019. The first column shows the observed number of crimes; the second column shows the predicted number of crimes; the third column shows the width of a prediction interval with a nominal coverage of 90\%.
}   
...
\begin{figure}[t!]
    ...
    \caption{Predictive distributions in the validation years (2010 and 2019) for three CAs (Archer Heights, Ashburn, and Roseland). The red and blue lines correspond respectively, to the observed and predicted number of crimes at each CA in the given year. 
    The first row corresponds to the year 2010; the second row corresponds to the year 2019. The first column corresponds to Archer Heights; the second column corresponds to Ashburn; the third column corresponds to Roseland. 
}   
...
We consistently observed that the empirical coverage in the year 2010 was slightly (4\% on average) higher than the nominal coverage, whilst it was lower (16\% on average) in the forecast year.
% For these validation years, we also computed the empirical coverage when using 90\%, prediction intervals, and the mean absolute percentage error (MAPE; see Appendix \ref{app:ScoringRules}). 
...
The time series of the observed data, predictions, and 90\% prediction intervals for these CAs is shown in Figure \ref{fig:chicago_time_series}. 
...
In this paper, we have described  \pkg{FRK} v2, an extension to the \proglang{R} package \pkg{FRK} that allows for the spatial and spatio-temporal modelling and large-scale prediction from big, non-Gaussian data. 
...
One requirement of the framework is that covariates need  to be known for every BAU, which may not be the case if covariates are recorded only at the data-support level. 
 Another requirement is the necessity to fix the fine-scale variance parameter in spatial change-of-support applications when \code{method = "TMB"}; note that this is not an issue if one is able to obtain a reliable estimate through other means (e.g., via previous census data or via quality-control experiments). 
 We are currently exploring avenues to relax this requirement, either through adjusting the model to allow estimation of the fine-scale variance parameter within \pkg{TMB}, or via a more robust offline estimate.
A limitation of \pkg{FRK} v2 is that despite the added flexibility, several models of interest, such as the zero-inflated Poisson, are still not catered for. 
 ...
 The main spatial data structures used in \pkg{FRK} v2 come from the package \pkg{sp} \citep{Pebesma_2005_sp_package}; future work may entail the inclusion of other spatial data structures, such as those from the package \pkg{sf} \citep{Pebesma_2018_sf_package}.
...
Recall from Section \ref{subsection:04-01:ProcessLayer} that \pkg{FRK} v2 allows the basis-function coefficients, $\vec{\eta}$, to be parameterised using either a prior covariance matrix, $\vec{K}$, or using a prior precision matrix, $\vec{Q}$.  
 ...
 Recall that both formulations use block-diagonal matrices, wherein basis-function coefficients with a resolution are dependent but they are independent different resolutions. Hence, we need only describe the intra-resolution dependencies. 
...
\end{equation}
where $d(\vec{s},\vec{s}^*)$ is a distance between locations $\vec{s}, \vec{s}^* \in D$.  
...
In \pkg{FRK} v2, we define $\beta_k$ to be proportional to the minimum distance between basis-function centroids; specifically, we set $\beta_k = \texttt{taper} \times \text{mindist}(k)$, where $\text{mindist}(k)$ is the minimum distance between the centroids of basis functions at the $k$th resolution and \code{taper} is a user-specified argument.
%Decreasing $\beta_k$ will increase the sparsity of 
%the tapered covariance matrix, %${\vec{K}_\textrm{tap}}_k$, 
% whilst increasing $\beta_k$ will decrease 
% %the sparsity of ${\vec{K}_\textrm{tap}}_k$.
...
\[
g(\mu(\cdot); k) = f(\pi(\cdot); k) = f(h(\mu(\cdot); k)) = (f \circ h)(\mu(\cdot); k) = Y(\cdot).
\]
Using a hierarchical-link-function approach with a negative-binomial data model and $f(\cdot)$ a logit function, we have that
\[
...
Third, our data do not make clear whether the head of the family is in the workforce; we therefore assume that the head of the family \textit{is} in the workforce, and hence we use the first half of Table 1 of the MIAESR guidelines.
