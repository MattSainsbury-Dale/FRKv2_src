...
%  Now define the vector of size parameters associated with the prediction regions, \mbox{\red{$D^P \equiv \{\tilde{B}_l : l = 1, \dots, N_P\}$}}, as
 ...
 If these size parameters are not available at unobserved BAUs, the user can still obtain predictions of $\vec{\pi}$ if a logit, probit, or complementary log-log functions is used, as in these cases $\vec{\pi}$ is linked directly to $\vec{Y}$ in (\ref{eqn:pi_i_equals_f_Y_i}). 
...
\begin{table}
 ...
 \caption{Important new or augmented function arguments in \pkg{FRK} v2.}\label{tab:new_arguments_in_functions} 
