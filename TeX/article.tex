...
%The preceding models used latent Gaussian random vectors within a GLMM framework; in contrast, Hu and Bradley (2018) used a spatio-temporal Pareto GLMM with latent log-Gamma random vectors (Bradley et. al, 2017) to model earthquake magnitudes.
...
The package \pkg{spNNGP} \citep{Finley_2020_spNNGP} facilitates dimension-reduction using a NNGP GLMM. %, and it carries out inference using an efficient data-augmented Gibbs sampler. 
...
The preceding discussion highlights some approaches and software packages that cater for a subset of the features that \pkg{FRK} v.2 is designed for. 
 To reiterate, \pkg{FRK} v.2 provides a unified framework that supports the modelling of large, point- and areally- referenced, non-Gaussian spatial and spatio-temporal data, and facilitates change of support over target supports that may be smaller or larger than the source supports (i.e., it caters for statistical down-scaling and up-scaling). Furthermore, it does so in a highly user-friendly manner, whereby challenging statistical problems may be tackled using only several lines of intuitive, readable code.
