...
In this paper, we take \pkg{FRK} to the next level where non-Gaussian data are treated by employing the generalised linear mixed model framework, and latent random effects are integrated out using an implementation of the Laplace approximation provided by the \proglang{R} package \pkg{TMB}. 
...
\Keywords{non-Gaussian spatial and spatio-temporal data, basis functions, \proglang{R}, change of support, areal data}
\Plainkeywords{non-Gaussian spatial and spatio-temporal data, basis functions, R, change of support, areal data}
...
\red{\cite{Bradley_2016_Bayesian_spatial_COS_lattice_data}: Bayesian Spatial Change of Support for Count-Valued Survey Data With Application to the American Community Survey.  \cite{Bradley_2018_computationally_efficient_multivariate_ST_models_for_high-dimensional_count-valued_data,  Bradley_2020_Bayesian_Hierarchical_Models_With_Conjugate_Full-Conditional_Distributions_for_Dependent_Data_From_the_Natural_Exponential_Family} modelled dependent data from the natural exponential family using the `conjugate multivariate distribution', which is the multivariate version of the conjugate prior for distributions in the natural exponential family presented by \cite{Diaconis_Ylvisaker_1979_conjugate_prior_natural_exponential_family}.} 
 \cite{Finley_2020_spNNGP} modelled binary data using a nearest neighbour Gaussian process \citep[NNGP; ][]{Datta_2016_NNGP_spatial} GLMM, making use of an efficient data-augmented Gibbs sampler whereby, through the use of Pólya-Gamma prior distributions \citep{Polson_2013_Polya-Gamma_Bayesian_logistic} on the random effects, the Gibbs updates are available in closed form.  
...
%To alleviate overfitting issues which may arise when assuming uncorrelated basis-function coefficients, \cite{Lee_2020_partitioned_domain_basis_function_non_Gaussian} used lasso regression to select a reduced set of basis functions. 
 \red{The preceding models used latent Gaussian random vectors within a GLMM framework; in contrast, Hu and Bradley (2018) used a spatio-temporal Pareto GLMM with latent log-Gamma random vectors (Bradley et. al, 2017) to model earthquake magnitudes.} 
...
A popular general purpose \proglang{R} package is \pkg{INLA} \citep{Lindgren_2015_R-INLA}, which uses the integrated nested Laplace Approximation \citep{Rue_2009_INLA} approach for model fitting and prediction, and caters for a number of non-Gaussian distributions. 
In principle, it is capable of tackling the difficult modelling challenges that come with big, areal, non-Gaussian spatial and spatio-temporal data. However, as \pkg{INLA} is not intended for this purpose, it can be difficult for a user to implement. 
...
Generalised additive models (GAMs), which rely on constructing smooth functions of the covariates (and in a spatio-temporal setting the covariates would include space and time),  can be implemented efficiently with the package \pkg{mgcv} \citep{Wood_2017_GAM:R}. 
The package yields excellent computational times, however it does not cater for areal data or spatial change-of-support.
The function \fct{spGLM} from the package \pkg{spBayes} \citep{Finley_2015_spBayes} fits univariate Bayesian generalised linear spatial regression models but with limitations: It only supports the modelling of binary and count (Poisson distributed) data in a non-Gaussian setting; it only caters for point-referenced data; and, as the basis functions depend on covariance-function parameters, computationally it can only handle a small number of predictive-process knots which causes a high degree of smoothing.
The package \pkg{spNNGP} \citep{Finley_2020_spNNGP} facilitates dimension-reduction using a NNGP \citep{Datta_2016_NNGP_spatial}. %, and it carries out inference using an efficient data-augmented Gibbs sampler. 
%(whereby, through the use of Pólya-Gamma prior distributions \citep{Polson_2013_Polya-Gamma_Bayesian_logistic} on the random effects, the Gibbs updates are available in closed form). 
 Currently \pkg{spNNGP} caters only for point-referenced spatial data, and it is limited to binomial data in a non-Gaussian setting. 
...
The statistical model used in \pkg{FRK} v.2 is a spatial GLMM \citep{Diggle_1998_spatial_GLMM}, a hierarchical model consisting of two layers.
...
The process layer, which governs the conditional mean of the data, retains many similarities to that in \pkg{FRK} v.1, as described by \cite{FRK_paper}. 
